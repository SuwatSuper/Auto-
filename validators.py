...
def _iv_check_sheet_day(clean_bills):
    ...
    for b in clean_bills:
        ...
        # [ADR-172/BUGHUNT] .isdecimal() แทน .isdigit(): digit ที่ int() รับไม่ได้ ('²','①') เดิมผ่านเช็ค
        #   แล้ว int() ครัช ValueError → bumper ตั้ง iv_seq/iv_date=[] ทั้งรอบ = การตรวจลำดับ IV ดับเงียบ
        #   ทุกไฟล์เพราะชีตเพี้ยนชีตเดียว. เลขไทย '๒' เป็น decimal → ผ่านเหมือนเดิม ; ASCII เดิมเป๊ะ.
        if sheet_raw.isdecimal():
...
def detect_iv_period_mismatch(iv_number, iv_date):
    ...
    if not m:
        return None
    prefix, lead = m.group(1), m.group(2)
...
def apply_sheet_date_crosscheck(all_bills):
    ...
    [ADR-057] guard sub-index: บางไฟล์ตั้งชื่อชีตเป็น "ลำดับบิลย่อย" ("5","5.1","5.2","5.3")
      ไม่ใช่ "วัน.เดือน". อาการ: "5.1" ถูกอ่านเป็น วัน05/เดือน01 ทั้งที่บิลข้างในเป็น 5 มี.ค.
      (= บิลย่อยลำดับ 1 ของวันที่ 5) → DOC001 false positive. เครื่องชี้ขาด: ในไฟล์เดียวกัน
      มีชีต "N" (เลขล้วน) อยู่ด้วย และวันที่ภายในตรงกับบิล "N.x" → ".x" คือลำดับย่อย ไม่ใช่เดือน.
      ตรวจทั้ง corpus 148 ไฟล์: เข้าเงื่อนไขนี้ไฟล์เดียว (TNT_69_03) → ลบเฉพาะ FP, true positive
      (TSH_68_0106 1.2/4.2, TKH_68_0112 11.10) คงครบ — zero collateral (พิสูจน์ด้วย sim_guard)."""
    # precompute: filepath -> {N(str): set(date)} ของชีตเลขล้วน (bare integer) เพื่อหา sibling ลำดับย่อย
    _bare_dates = {}
    for _b in all_bills:
        _sh = str(_b.get('sheet', '')).strip()
        # [ADR-172/BUGHUNT] .isdecimal() — เหตุผลเดียวกับ _iv_check_sheet_day: '²'/'①' ทำ int() ครัช
        #   ตั้งแต่ precompute (ก่อนถึงบิลใด) → DOC001 crosscheck หายทั้งรอบ.
        if _sh.isdecimal() and _b.get('iv_date'):
            _dt = _b['iv_date']
            _bare_dates.setdefault(_b.get('filepath'), {}).setdefault(str(int(_sh)), set()).add((_dt.year, _dt.month, _dt.day))
    for b in all_bills:
        ...
        if sheet_match:
            ...
            _bd = b['iv_date']
            if _sib and (_bd.year, _bd.month, _bd.day) in _sib:
                continue

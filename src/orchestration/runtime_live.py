...
class _LiveTradingMixin(_RuntimeBase):
    ...
    def _ensure_manual_lock(self) -> asyncio.Lock:
        """Lazily create the per-runtime lock that serializes operator orders.

        Two concurrent ``manual_order`` calls (e.g. a double-clicked BUY) must
        not both pass the single-position check and place TWO real bids before
        either records the position. The lock is held across the whole
        read-decide-place-mutate section, including the network bid (M3)."""
        lock = self._manual_order_lock
        if lock is None:
            lock = asyncio.Lock()
            self._manual_order_lock = lock
        return lock
    ...
    async def manual_order(
        ...
        if side_u not in ("BUY", "SELL", "CLOSE"):
            return False, {"error": f"unknown side {side!r} (use BUY/SELL)"}
        # M3: serialize the entire read-decide-place-mutate section so two
        # concurrent operator clicks can't both place a REAL order before the
        # paper position is set. The lock is held across the network bid.
        async with self._ensure_manual_lock():
            if side_u == "BUY":
                # The breaker blocks a NEW entry only when it should halt (live /
                # opted-in paper). Paper keeps trading through a losing streak. A
                # CLOSE below is NEVER gated — you must always be able to exit.
                if (
                    ...
                    return False, {
                        "error": "circuit breaker is OPEN — reset it to open new positions"
                    }
                # In live-armed mode a manual BUY fires a REAL bid first; the
                # paper position is then marked live-backed so its exits close
                # the real position too. Manual orders bypass the p_win gate.

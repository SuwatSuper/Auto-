...
class _AgentsMixin(_RuntimeBase):
    ...
    def _make_agents(self) -> dict[str, AgentLike]:
        ...
        # Persist the breaker on every state change so a TRIPPED breaker survives
        # a restart (C1) — the operator, not a silent reboot, must reset it.
        self._circuit_breaker.on_change = self._schedule_breaker_persist
        ...
        agents: dict[str, AgentLike] = {
            ...
            "risk_gate": ExecutionAgent(
                ...
                # Critical-alert sink for an AMBIGUOUS live order result (C2).
                alert_fn=self._schedule_alert,

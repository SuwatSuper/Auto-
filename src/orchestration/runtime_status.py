...
class _StatusMixin(_RuntimeBase):
    ...
    def write_daily_summary(self) -> dict[str, object]:
        ...
        if tr is None:
            return {}
        # M10: keep the whole row on ONE basis — the realized ledger (as the
        # module promises: "every field derived from the real treasury ledger,
        # net of fees"). end_equity = start + realized_today, so the equity
        # columns reconcile with pnl_net / pct_return instead of mixing in
        # unrealized mark-to-market only on end_equity (which made
        # end_equity − start_equity ≠ pnl_net while a position was open).
        row = build_summary_row(
            date=tr.day_key,
            # Per-DAY values so each row reflects that day, not lifetime totals.
            start_equity=tr.day_start_equity,
            end_equity=tr.day_start_equity + tr.realized_today,

...
class ExecutionAgent:
    ...
    def __init__(
        ...
        # Critical-alert sink (message, level). Used to surface an AMBIGUOUS live
        # order result (transport/timeout) that may have left a real position the
        # bot can't see — operator must reconcile manually (C2).
    ...
    async def _route_live(self, data: dict[str, object]) -> None:
        ...
        try:
            ...
            self._release_inflight(signal)
            self._handle_order_exception(exc, str(action), sym, amount, rate)
            return
        ...
        # M4: a resting LIMIT bid that returned NO confirmed fill must NOT be
        # mirrored as a held paper position — we'd be inventing coin we don't own,
        # and a later protective close would try to sell what isn't there. We
        # can't poll order_info to learn if it fills, so surface it for the
        # operator instead. Market orders (the default) always echo a fill, so
        # they're unaffected.
        if (
            action == "bid"
            and str(typ).lower() == "limit"
            and filled in (None, "", "0", 0)
        ):
            self._log.warning(
                "execution_agent.live_limit_resting",
                order_id=order_id, symbol=sym, amount=amount, rate=rate,
            )
            return
    ...
    def _handle_order_exception(
        self, exc: Exception, action: str, sym: str, amount: object, rate: object
    ) -> None:
        """Classify a failed live order: explicit exchange rejection vs ambiguous.

        - ``BitkubApiError``: the exchange RECEIVED the request and rejected it
          with an error code → no fill happened, safe to drop and keep trading.
        - any other (transport/timeout/HTTP) error: AMBIGUOUS — Bitkub may have
          received and FILLED the order while the reply was lost. We can't poll
          order-info without an order id, so we must NOT keep firing blind while
          possibly holding an untracked real position. HALT the live engine by
          tripping the breaker (which now survives a restart, C1) and raise a
          CRITICAL alert for manual reconciliation (C2).
        """
        from infrastructure.gateway.bitkub_rest import BitkubApiError  # noqa: PLC0415

        if isinstance(exc, BitkubApiError):
            self._log.error(
                "execution_agent.live_order_rejected", code=exc.code, error=str(exc)
            )
            return
        self._log.critical(
            "execution_agent.live_order_ambiguous",
            action=action, symbol=sym, amount=amount, rate=rate, error=str(exc),
        )
        with contextlib.suppress(Exception):
            self._breaker.trip("AMBIGUOUS_ORDER_RESULT", int(time.time()))
        if self._alert_fn is not None:
            with contextlib.suppress(Exception):
                self._alert_fn(
                    f"⚠️ LIVE ORDER RESULT UNKNOWN — {action} {sym} amt={amount} "
                    f"rate={rate}: {exc}. Trading HALTED (breaker tripped). Check "
                    "Bitkub order history and reconcile/cancel manually before "

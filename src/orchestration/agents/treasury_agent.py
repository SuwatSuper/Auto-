...
class TreasuryAgent:
    ...
    def release_reservation(self, amount: Decimal) -> None:
        ...
        Also rolls back the per-day entry quota and approval counters that
        ``request_open`` incremented, so a reservation that never became a
        position doesn't burn a day's trade slot or inflate approval stats
        (L2). Counters never drop below zero."""
        self.cash += amount

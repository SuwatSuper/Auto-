...
def _dec(value: object, default: Decimal = Decimal("0")) -> Decimal:
    """Best-effort Decimal conversion (never raises, never returns non-finite).

    Used on money paths (e.g. the trailing-stop level) so the value stays a
    full-precision Decimal instead of round-tripping through float and picking
    up sub-satang noise — the codebase's "Decimal only on money paths" rule.
    """
    try:
        if value is None:
            return default

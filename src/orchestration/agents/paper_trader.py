...
class PaperTraderAgent:
    ...
    async def _close(self, mark: Decimal, reason: ExitReason) -> None:
        ...
        if self._circuit_breaker is not None:
            # Pass the real epoch-seconds so an auto-trip's audit entry carries
            # the actual time, not 1970 (L7).
            self._circuit_breaker.record_trade(trade.pnl, int(time.time()))

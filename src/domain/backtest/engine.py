...
def run_backtest(
    ...
    symbol = "THB_BTC"
    # H5: per-ROUND-TRIP net PnL (captured when the position returns to flat) so
    # win_rate / expectancy describe TRADES, not green/red bars.
    realized_trade_pnls: list[Decimal] = []
    last_flat_equity = initial_cash.amount
    ...
    for i in range(len(prices) - 1):
        ...
        # H5: capture per-round-trip net PnL when the position returns to flat —
        # equity then equals cash, so the delta since the last flat point is the
        # realized result net of fees.
        pos_now = positions.get(symbol)
        flat = pos_now is None or pos_now.qty == 0
        if flat:
            if was_in_position:
                realized_trade_pnls.append(eq.amount - last_flat_equity)
            last_flat_equity = eq.amount
            was_in_position = False
        else:
            was_in_position = True

        # H5: feed the daily-loss gate fee-NET realized PnL. apply_fill's
        # realized is GROSS (fees are deducted from cash, not from realized_pnl),
        # so without this the backtest's risk model is more permissive than
        # reality by the cumulative fees paid. (This figure is cumulative since
        # inception — the engine has no calendar-day boundaries — so the cap
        # behaves as a cumulative-loss fence, which is what evaluate() enforces.)
        daily_pnl = Money(
            amount=account.realized_pnl.amount - total_fees,
            currency=initial_cash.currency,
        )
    ...
    # H5: win_rate / expectancy over completed ROUND-TRIPS (net of fees), not
    # per-bar equity deltas (which scored green/red BARS as if they were trades
    # and inflated the win rate that the live entry gate then trusts).
    pnls_for_stats = [p for p in realized_trade_pnls if p != 0] or [Decimal(0)]
    wr = win_rate(pnls_for_stats)
    exp = expectancy(pnls_for_stats)

...
// Emotion state is derived from real agent counters (running, stale, restarts)
function tickEmotions() {
  AGENT_DEFS.forEach(def => {
    const agent = AppState.agents[def.id];
    if (!agent) return;
    const running = agent.running ?? false;
    const stale   = agent.stale   ?? false;
    const restarts = agent.restarts ?? 0;
    // Derive emotions from real status
    const stress     = stale ? 80 : (running ? Math.min(20 + restarts * 10, 90) : 50);
    const fatigue    = running ? Math.min(10 + restarts * 5, 80) : 30;
    const morale     = running && !stale ? Math.max(90 - restarts * 5, 40) : (stale ? 30 : 50);
    const confidence = running && !stale ? Math.max(85 - restarts * 8, 30) : 25;
    updateAgentEmotion(def.id, { fatigue, stress, morale, confidence });
  });
}

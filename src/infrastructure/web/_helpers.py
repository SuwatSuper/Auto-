...
def check_csrf(request: Request) -> None:
    """Reject cross-site state-changing requests (CSRF defense — H1).

    The local control plane trusts loopback requests without a key, which leaves
    it reachable by a malicious web page the operator merely opens in a browser:
    the browser will happily POST cross-origin to http://127.0.0.1:8000/api/*
    from loopback (a 'simple request' with Content-Type text/plain dodges the
    CORS preflight, and CORS never blocks the state change itself). We stop that
    using the browser-set ``Origin`` / ``Sec-Fetch-Site`` headers.

    Non-browser clients (curl, the bot's own scripts, the test client) send
    neither header and are unaffected; the same-origin dashboard sends matching
    ones and passes. Only unsafe methods are gated — GET/HEAD/OPTIONS never
    change state.
    """
    ...
    if origin:
        ...
        if urlparse(origin).netloc != host:
            raise HTTPException(
                status_code=403, detail="cross-origin request blocked (CSRF)"
            )
...
def check_api_key(request: Request, runtime: PipelineRuntime) -> None:
    ...
    network (and DASHBOARD_API_KEY is set). A cross-site browser request is
    rejected even on loopback (CSRF defense) before the localhost shortcut.
    """
    check_csrf(request)
...
def check_api_key_strict(request: Request, runtime: PipelineRuntime) -> None:
    ...
    cap, the kill switch, and the treasury / circuit-breaker gates. A cross-site
    browser request is rejected even on loopback (CSRF defense) before the
    localhost shortcut.
    """
    check_csrf(request)
    if is_local_request(request):

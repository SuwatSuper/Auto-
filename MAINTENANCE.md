# MAINTENANCE — กฎโดยนัยที่คนดูแลต้องรู้ (ปุ้มปุ้ย v9.3.5)
...
1. **Golden hash ห้ามขยับโดยไม่ตั้งใจ.** ผลตรวจ (audit decision) ถูก fingerprint ด้วย SHA256.
   ...
   PYTHONHASHSEED=0 PUOPUY_AUDIT_DATE=2026-06-02 \
     python3 regression_full.py . tests/fixtures tests/fixtures/baseline_fixture.json   # d05e5de3… (fixture)
     python3 golden_master.py . /tmp/x.json /mnt/project                                # 814c7cf0… (146 ไฟล์ ทางการ)
     python3 regression_full.py . <106-ไฟล์จริง>                                         # 814c7cf0… (ทางการ)
   ```
...
- ⏳ **(รอ golden gate 146 ไฟล์ + อนุมัติ)** ลด hot loop ใน parser (`_dic_int_run`/`_row_label_match`) เพื่อ perf — แตะ parse core (byte-sensitive) ห้ามแก้แบบเดา.

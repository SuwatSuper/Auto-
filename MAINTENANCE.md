...
1. **Golden hash ห้ามขยับโดยไม่ตั้งใจ.** ผลตรวจ (audit decision) ถูก fingerprint ด้วย SHA256.
   ...
   PYTHONHASHSEED=0 PUOPUY_AUDIT_DATE=2026-06-02 \
     ...
     python3 golden_master.py . /tmp/x.json /mnt/project                                # f05358aa… (152 ไฟล์ ทางการ)
     python3 regression_full.py . <106-ไฟล์จริง>                                         # f05358aa… (ทางการ)
   ```
...
- ⏳ **(รอ golden gate 152 ไฟล์ + อนุมัติ)** ลด hot loop ใน parser (`_dic_int_run`/`_row_label_match`) เพื่อ perf — แตะ parse core (byte-sensitive) ห้ามแก้แบบเดา.

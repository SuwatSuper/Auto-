...
if len(FILES) == 81:
    ...
    check(sig == (632, 1, 26, 0, 34, 2),
          f"ผลหลักตรง baseline 81 ไฟล์ (bills/dup/iv_seq/iv_date/typos/companies) = {sig}")
else:
    print(f"  • ข้าม (ชุดข้อมูลนี้มี {len(FILES)} ไฟล์ ไม่ใช่ 81 — baseline counts ใช้ไม่ได้)")

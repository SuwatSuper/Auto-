...
Scope: merge `kingdom_prime_dashboard.html` into the backend, wire the 7 real
department agents into the runtime, secure the control plane with X-API-Key.

Defects found & fixed:
1. SqliteStateStore cross-thread crash (1 failing test) → check_same_thread=False under existing lock
2. status() lacked every field the Kingdom dashboard reads → added (additive; all old keys kept)
3. Runtime ran 3 SampleAgents; the 7 department agents existed but were never wired → wired 1:1 with dashboard chibis (H5)
4. /metrics read wrong status keys (always 0) → fixed via status aliases, pinned by test
5. kingdom dashboard absent from project → served at `/` (old UI at `/classic`), key injected server-side
6. scripts/run.py double-started the runtime (duplicate price supervisor) → lifespan-only start
7. Control endpoints unauthenticated → POST /api/* requires X-API-Key when DASHBOARD_API_KEY set (hmac.compare_digest)

Evidence (real runs):
- pytest: 210 passed, 0 failed (was 201 passed / 1 FAILED); +8 integration tests in tests/web/test_kingdom_integration.py
- ruff: All checks passed!
- mypy --strict: Success: no issues found in 68 source files
- Live smoke: /healthz ok; / serves Kingdom Prime with key injected (placeholder count 0);
  /classic serves old UI; auth no-key→401 bad-key→401 ok-key→200;
  WS streams types {price,status}; 7 agents running; simulator ~5 msg/s
- Honesty: equity == initial_capital, pnl_today == 0, win_rate == null until first real
  rolling-backtest window — no fabricated numbers (paper trading only)

## PY312 ALIGNMENT (2026-06-12)
- Offline wheels rebuilt: cp311-win_amd64 → cp312-win_amd64 (orjson, pydantic_core, websockets) + colorama added
- Completeness proven by marker simulation for Windows/CPython 3.12.10: 22 wheels, 27 deps checked, 0 missing
- requires-python ">=3.11" → ">=3.12"; uvicorn[standard] → uvicorn (websockets already direct dep); .python-version=3.12
- Launchers prefer `py -3.12` / python3.12; suite re-verified on 3.12: 210 passed

## Phase 4 — Paper Trading (1,000 THB) + Account Guardian + H5 Truth Layer (2026-06-12)

| Gate | Result |
|---|---|
| Full test suite | **241 passed** (×3 consecutive runs) |
| ruff (src+tests+scripts) | clean |
| mypy --strict (72 files) | clean |
| Live smoke (35s, tight brackets) | equity 1000 → 964.54, 6 trades closed, ledger 0W/6L, daily-loss 3.55% tracked |
| Restart-survival proof | open position + ledger resurrected from SQLite; bracket closed it post-reboot (closed 1→2) |
| Paper-only guard | tests/architecture/test_paper_only.py scans src — any live-order code turns CI red |

Fixed this phase: real chibi metrics (no Math.random), heartbeat + stale/crash truth (H5),
watchdog auto-restart with crash reasons, SQLite persistence ON by default,
PaperTraderAgent (bracketed, fee+slippage-true), TreasuryAgent (sole cash authority:
survival floor 700฿, daily-loss cap 50฿, veto chain). Known limits: single symbol (THB_BTC),
LIVE Bitkub feed untested from sandbox (no order path exists by design).
...
## ✅ FINAL COMPLETION — Items 1–10 (claude/clever-hamilton-bf7qv5)

| # | Item | Fix | Proving Test |
|---|------|-----|-------------|
| 1 | P0-1 · Risk gate actually gates trades | `ExecutionAgent` built in `runtime._make_agents` with `CircuitBreaker`+`TokenBucket`; `PaperTraderAgent` subscribes to `decisions.approved.v1`; `record_trade` called after every close | `tests/orchestration/test_risk_gates_trades.py` (5 tests) |
| 2 | P0-2 · Unbounded memory fixed | `entry_exit.py` and `probability.py` cap `_prices` to 500 after every append | `tests/orchestration/test_memory_bounds.py` (4 tests) |
| 3 | P0-3 · Atomic crash-safe state | Combined `paper.session.v1` written atomically after every open/close; on load validates consistency (drops phantom position, keeps cash) | `tests/orchestration/test_crash_recovery.py` (3 tests) |
| 4 | P1-1 · Thai local time for daily risk | `TreasuryAgent` accepts `tz_offset_minutes=420`; `_local_day()` uses stdlib `datetime.timezone`; rollover called on `request_open` and `settle_close` | `tests/orchestration/test_thai_time.py` (4 tests) |
| 5 | P1-2 · Open-position cap enforced | `ExecutionAgent` checks `open_positions_fn()` and vetoes BUY when cap reached | `test_position_cap_blocks_second_buy` in test_risk_gates_trades.py |
| 6 | P1-3 · Bound `_seen_ids` | `collections.deque(maxlen=10000)` + companion set; evicts oldest on overflow | `tests/orchestration/test_seen_ids_bounded.py` (2 tests) |
| 7 | P1-4 · Count dropped bus messages | `InMemoryEventBus.dropped_messages` counter; surfaced in `runtime.status()` and dashboard badge | `tests/infrastructure/test_dropped_messages.py` (3 tests) |
| 8 | P1-5 · SQLite WAL durability | `PRAGMA journal_mode=WAL; synchronous=NORMAL; busy_timeout=5000` on connect | `tests/infrastructure/test_sqlite_wal.py` (2 tests) |
| 9 | P2-3 · Agent cards show real activity | Removed `initialMetrics: {morale:82,...}` and `Math.random()` jitter; cards driven from `State.agents[id]` real fields | `grep -rnE "0\.1245\|Math\.random\|morale: 82" src/infrastructure/web/static/` → 0 matches |
| 10 | P3-3 · CI + coverage gate | `.github/workflows/ci.yml` runs ruff+mypy+pytest with `--cov-fail-under=85` | CI green |

### Final Test Results
```
451 passed, 1 skipped in ~25s
ruff check src tests  →  All checks passed!
mypy src/             →  Success: no issues found in 93 source files
grep for fake data    →  0 matches
```

### Architecture Guards (still green)
- `tests/architecture/test_layer_rules.py` — 13 passed
- `tests/architecture/test_execution_guard.py` — 9 passed
- Execution remains PAPER; 4 gates unchanged; `place_bid`/`place_ask` never called automatically

...
def load_master():
    ...
    try:
        ...
        # [ADR-165] ห้ามเงียบ: ไฟล์ "มีอยู่แต่อ่านพัง" ≠ "ยังไม่กรอก" — ถ้าเงียบ เจ้าของจะคิดว่า
        #   กฎตัวตน (~24 ข้อ) ทำงานอยู่ทั้งที่หลับ. ระบบยังรันต่อได้ (แบบไม่มี master) แต่ต้องบอกดังๆ.
        print(f"🚨 master_companies.json มีอยู่แต่อ่านไม่ได้ ({type(e).__name__}: {str(e)[:60]})")
        print("   → กฎตัวตน (~24 ข้อ) จะไม่ทำงานจนกว่าจะแก้ไฟล์ (แก้ JSON หรือรัน เพิ่ม_master.py ใหม่)")
        return None
    if not isinstance(data, dict):
        # [ADR-165] กันชนิดผิด (เช่นมือแก้เป็น list) หลุดไป crash ปลายทาง
        print(f"🚨 master_companies.json ต้องเป็น JSON object ({{...}}) แต่เป็น {type(data).__name__}"
              " → ระบบทำงานแบบไม่มี master จนกว่าจะแก้")
        return None
    if data.pop('_golden_stub', False):
    ...
    if bad:
        print(f"⚠  master: {len(bad)} รายการรูปแบบไม่ถูก (value ไม่ใช่ object) เช่น {bad[:3]}"
              " — รายการเหล่านี้อาจถูกข้ามตอนตรวจ (แก้ด้วย เพิ่ม_master.py)")
    return data

...
python3 regression_full.py . tests/fixtures tests/fixtures/baseline_fixture.json   # ต้อง d05e5de3

# ชั้น corpus จริง (ผูก /mnt/project):
python3 verify_corpus_manifest.py /mnt/project    # MATCH = code ต้องถูก / DRIFT = data เปลี่ยน
bash run_ci.sh /mnt/project                        # regression เต็ม (ต้อง 814c7cf0)
```
...
python3 backup_kit.py build puopuy_v9_3_4_OFFLINE_COMPLETE_814c7cf0_5year.zip /mnt/project
...
```
ขั้นกู้เต็ม: เปิด `RESTORE_TH.md` ใน bundle (7 ขั้น พร้อม hash ที่ต้องได้: fixture `d05e5de3…` → real `814c7cf0…`)

## ⛔ ห้ามเด็ดขาด

- `pythainlp`: ไม่บังคับ — **ถ้ามีต้อง 5.0.5 เป๊ะเท่านั้น** (พิสูจน์ 2026-07-02: มี 5.0.5 → golden 814c7cf0 เป๊ะเดิม, ADR-164) · เวอร์ชันอื่นห้ามจนกว่าจะพิสูจน์ (gate จะ FAIL ให้เองที่ major)

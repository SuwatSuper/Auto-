...
# [ADR-102] golden_snapshot.MASTER ว่างแล้ว (ลบบริษัทตัวอย่าง ฉี อัน ตามคำสั่งเจ้าของ) →
#   เทสนี้นิยาม master fixture ของตัวเองเพื่อทดสอบ master-matching path (ไม่พึ่ง MASTER กลาง)
MASTER = {
    "ฉี อัน คอนสตรัคชั่น": {
...
for i, r in enumerate(rows, 1):
    for ln in V.render_block(i, r).splitlines():
        # [ADR-099/ADR-102] ปัญหาเว้นวรรค (CMP004) ของบิลที่ "ตรงกับ master canonical" surface ที่บรรทัด
        #   "ตรวจตาเพิ่ม (ชื่อบจ.)" (ช่อง 'ชื่อบจ.' = 'ตรง' เพราะชื่อตรงระดับ canonical; spacing ก้ำกึ่ง → ขอตาคน)
        if 'ชื่อบจ' in ln and 'เว้นวรรค' in ln:
            name_line = ln

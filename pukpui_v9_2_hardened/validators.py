...
#   → เติมครบเหมือนเดิมทุกตัว (hash ไม่ขยับ — พิสูจน์ด้วย d8bcde85/f1ac8421/fff69fc6).
...
def _iv_check_sequence(consistent_bills):
    ...
    for (file_key, vendor, date_key), group in groups.items():
        # v5.8L: IV ในกลุ่มยาวไม่เท่ากัน = format ต่างกัน → ไม่เทียบ
        seq_lengths = set(len(str(e['_seq'])) for e in group)
...
def _iv_check_ascending(clean_bills):
    ...
    for b in clean_bills:
        ...
        if not m_seq:
            continue
        vendor = clean_tax_id(b.get('tax_id','')) or normalize_text(b.get('company','')).upper()
        asc_groups[(b.get('file',''), vendor)].append(
            (b['iv_date'], int(m_seq.group(1)), str(b['iv_number']).strip(), b))

    for (file_key, vendor), entries in asc_groups.items():
        # ความยาวเลขท้ายต้องเท่ากัน (กัน format ต่างกันมาเทียบมั่ว)
        if len({len(str(n)) for _, n, _, _ in entries}) > 1:
            continue
        entries.sort(key=lambda x: x[0])
        if len(entries) < 2:
            continue
        prev_d, prev_n, prev_iv, _prev_b = entries[0]
        for d, n, iv, b in entries[1:]:
...
def check_iv_date_sequence(bills):
    ...
    for b in bills:
        if not b['iv_number'] or not b['iv_date']: continue
        ...
        if last_num is not None:
            ...
            by_group[month_key].append((b['iv_date'], last_num, b))
...
def check_filename_consistency(filename, bills, file_info):
    ...
    dates = [b['iv_date'] for b in bills if b['iv_date']]

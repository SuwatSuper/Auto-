...
RELIABLE_CODES = {
    ...
    "DOC001", "DOC002", "DOC003", "DT004", "DT005",     # ลงวันที่ผิด / เลขเอกสารซ้ำ / บิลไม่มีวันที่
    "ITM001", "ITM002", "ITM016", "ITM017", "ITM018",   # ยอดบรรทัด/ลำดับ/ซ้ำ/ติดลบ/จำนวน0
    "IV002", "IV003", "IV005",                          # เลขใบกำกับผิดรูปแบบ / ซ้ำข้ามวัน / ไม่มีเลขที่
    "TAX001", "TAX002", "TAX006",                       # เลขภาษี ความยาว/อักษรปน/checksum
    "VAT001", "VAT002", "VAT003", "VAT005", "VAT007", "VAT009",  # ยอด/VAT เชิงเลขคณิต
}
...
FUZZY_CODES = {
    ...
    "ITM010", "ITM011", "ITM012", "ITM013", "ITM014", "ITM015",  # ชื่อ/หน่วย/typo/ลำดับ(soft)
...
def emit_ultra_summary(bills, outdir, master_present=True):
    ...
    L = ["=" * 64,
         ...
         f"สร้างเมื่อ: {datetime.now().strftime('%d/%m/%Y %H:%M')}",
         f"รวม {len(vcs)} บริษัท/เดือน  |  พร้อมส่ง {n_clean}  |  ยืนยันผิด {n_conf}  |  "
         f"ควรตรวจซ้ำ {n_rech}  |  น่าจะปกติ {n_okish}",

...
from pukpui_modular_base import (   # [SPLIT #3] shared scope (explicit)
    ...
    apply_iv_period_crosscheck, apply_missing_date_check, apply_sheet_date_crosscheck, audit_text_num_reset, build_dashboard_figs,
...
def _emit_company_summary(all_bills, report_dir, master_present=True):
    ...
    try:
        ...
        print(f'   {len(_rows)} บริษัท×เดือน: ต้องแก้ {_nfix} / ตรง {_nclean}')
...
def _is_real_master(master) -> bool:
    ...
    golden/regression (golden_master.py / verify_golden.py / regression_oracle.py) เขียน
    master_companies.json จากชุดทดสอบ (golden_snapshot.MASTER) ทิ้งไว้ในโฟลเดอร์ได้ —
    ฟังก์ชันนี้กัน foot-gun: รัน golden เสร็จแล้วมากด Run ต่อ จะได้ไม่หยิบ stub มาตรวจจริง
    โดยไม่รู้ตัว. คืน False เมื่อ: ว่าง/None หรือ 'ทุกบริษัทในไฟล์' ตรงกับ stub (ตามเลขภาษี).
    ...
    for v in master.values():
        if (v or {}).get('tax_id') not in _stub_tax:
            return True   # มีบริษัทที่ไม่ใช่ stub → ของจริง
    return False          # ทุกบริษัทตรง stub → ถือเป็น stub ทดสอบ
...
def _move_processed_files(file_list, data_dir, report_dir):
    ...
    if not (file_list and data_dir):
        return
    done_dir = os.path.join(report_dir, f'ตรวจแล้ว_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    try:
    ...
    for src in file_list:
        try:
            if not os.path.isfile(src):
                continue
            # ย้ายเฉพาะไฟล์ที่อยู่ในโฟลเดอร์ต้นทางจริง (กันย้ายไฟล์นอกขอบเขตโดยไม่ตั้งใจ)
            if os.path.abspath(os.path.dirname(src)) != _data_abs:
                continue

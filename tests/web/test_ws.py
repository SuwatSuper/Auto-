...
def _make_runtime() -> PipelineRuntime:
    settings = Settings(training_mode=False, persist_state=False, initial_capital="1000", dashboard_api_key=SecretStr(""), prices_topic="prices.thb_btc.v1")
...
def test_ws_origin_matching_host_is_accepted() -> None:
    """A connection with Origin equal to the request Host header is accepted."""
    runtime = _make_runtime()
    app = create_app(runtime)
...
def test_ws_bogus_origin_is_rejected() -> None:
    """A connection from a cross-origin host is rejected with 4403."""
    runtime = _make_runtime()
    app = create_app(runtime)

    with TestClient(app) as client, pytest.raises(Exception), client.websocket_connect(  # noqa: B017

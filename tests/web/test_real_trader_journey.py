...
async def test_paper_live_toggle_is_token_gated() -> None:
    ...
    async with _client(rt) as c:
        ...
        # set a per-order cap AND real backstops (M1: a finite consecutive-loss
        # breaker + daily-loss cap < 100 are required before live), then arming
        # live succeeds — exactly the order an operator configures it.
        await c.post(
            "/api/risk/settings",
            json={
                "max_single_order_thb": "500",

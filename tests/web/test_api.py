...
def _make_runtime() -> PipelineRuntime:
    settings = Settings(training_mode=False, persist_state=False, initial_capital="1000", prices_topic="prices.thb_btc.v1", dashboard_api_key=SecretStr(""))
...
@pytest.mark.asyncio
async def test_mode_switch_accepts_live(runtime_client) -> None:  # type: ignore[no-untyped-def]
    _, client = runtime_client
    resp = await client.post("/api/mode/live")
    assert resp.status_code == 200
    assert resp.json()["mode"] == "live"


@pytest.mark.asyncio
async def test_mode_switch_rejects_simulator(runtime_client) -> None:  # type: ignore[no-untyped-def]
    """Production Migration: simulator/paper/demo modes are gone."""
    _, client = runtime_client

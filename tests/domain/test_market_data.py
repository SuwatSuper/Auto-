...
def test_missing_timestamp_uses_now_ms() -> None:
    """Bitkub ticker carries no timestamp; now_ms is used as event time."""
    raw = _valid_raw()
    del raw["ts"]
    result = normalize_bitkub_ticker(raw, now_ms=_NOW_MS)
    assert isinstance(result, PriceUpdate)
    assert result.ts_ms == _NOW_MS
...
def test_golden_official_bitkub_payload() -> None:
    """Golden test using the official Bitkub market.ticker payload (no ts field)."""
    ...
    result = normalize_bitkub_ticker(payload, now_ms=1700000000000)
    assert isinstance(result, PriceUpdate)
    assert result.price == Decimal("2883194.85")
    assert result.symbol == "THB_BTC"

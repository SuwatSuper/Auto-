...
def test_learning_today_uses_thai_day_not_host_clock(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """Regression: the per-agent learner day-key must roll at THAI midnight, not
    the host's local/UTC midnight, so it stays aligned with the treasury on a
    non-Bangkok host. 18:30 UTC == 01:30 Thai the NEXT day. Before the fix,
    learning._today() used naive datetime.now() and reported the UTC day.
    """
    from orchestration.agents import learning

    fixed_utc = datetime(2026, 1, 1, 18, 30, tzinfo=UTC)
    ...
    monkeypatch.setattr(learning, "datetime", _FakeDateTime)
    assert learning._today() == "2026-01-02"  # Thai day, not UTC's 2026-01-01

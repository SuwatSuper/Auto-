...
@pytest.mark.asyncio
async def test_trailing_stop_pushes_exact_decimal_no_float_noise() -> None:
    """Regression (money-path rule): the trailing-stop level pushed onto the
    (possibly live) position must be an EXACT Decimal, never a float that picked
    up sub-satang noise. peak * (1 - 1.5%) for 2345678.01 is exactly
    2310492.83985; the old float path produced 2310492.8398499996."""
    pushed: list[object] = []

    class _RT:
        def status(self) -> dict[str, object]:
            return {"portfolio": [{"mark_price": "2345678.01"}]}

        def update_trailing_stop(self, stop: object) -> bool:
            pushed.append(stop)
            return True

    agent = TrailingStopBotAgent("trailing_stop", _RT(), 1.5, _LOG)
    await agent.tick()

    assert pushed, "trailing stop was not pushed"
    value = pushed[0]
    assert isinstance(value, Decimal)
    assert value == Decimal("2310492.83985")
    assert value.is_finite()
    # The agent's own field is Decimal too (no float twin).
    assert isinstance(agent.trail_stop, Decimal)

...
# ============================================================================
# SELF-TESTS — รวมอยู่ในไฟล์เดียวกับระบบ (ไม่ต้องมีไฟล์แยก)
#   รันเทสต์:  python set_edge_finder.py --selftest      (หรือ) pytest set_edge_finder.py
#   รันระบบ :  python set_edge_finder.py                  (ส่วนนี้ไม่ทำงาน ไม่ต้องมี pytest)
# ============================================================================
import sys as _sys
sef = _sys.modules[__name__]          # alias: โค้ดเทสต์อ้าง sef.<ชื่อ> = โมดูลนี้เอง
try:
    import pytest                      # ใช้เฉพาะตอนรันเทสต์ (รันระบบปกติไม่ต้องมี)
except ImportError:
    pytest = None

if pytest is not None:
    @pytest.fixture(autouse=True)
    def _unit_tests_disable_friday_anchor():
        """unit tests รันโดยปิด Friday anchor (มีเทสต์ของ mask แยกไว้เอง);
        ทำผ่าน fixture แทน global mutation -> ไม่กระทบค่าเริ่มต้นตอนรันระบบจริง."""
        _old = sef.FRIDAY_ONLY
        sef.FRIDAY_ONLY = False
        yield
        sef.FRIDAY_ONLY = _old
...
if __name__ == "__main__":
    import sys
    if any(a in ("--selftest", "--test") for a in sys.argv[1:]):
        if pytest is None:

"""
================================================================================
SET WEEKLY STAT-EDGE FINDER v2 — MULTI-STRATEGY
single file, copy-paste into Google Colab, run. ดึงข้อมูลสดตอนกดรันเท่านั้น.
================================================================================
เหตุผลที่เขียนใหม่ (Root Cause จากผลรันจริง 3 รอบ):
  กลยุทธ์เดิม (RSI+BB mean-reversion อย่างเดียว + TP/SL % ตายตัว) หา WR>70% &
  กำไร>2% ไม่เจอเลยจาก 3.2M combo เพราะ (1) จับแค่ mean-reversion (2) เป้า %
  ตายตัวไม่ปรับตามความผันผวนจริงของแต่ละหุ้น.

v2 แก้ด้วยการเพิ่ม "ตระกูลกลยุทธ์" (entry family) และ exit แบบ ATR:
  ENTRY families (เลือกได้ใน 1 combo):
    0 = MEAN_REVERSION : RSI<threshold AND Close<BB_lower  (ของเดิม)
    1 = BREAKOUT       : Close > Donchian high(N)          (ซื้อตามโมเมนตัม)
    2 = TREND_PULLBACK : EMA_fast>EMA_slow AND RSI dips<threshold (ย่อในเทรนด์ขึ้น)
    3 = MACD_CROSS     : MACD line cross above signal
  EXIT modes:
    TP/SL แบบ ATR-multiple (ปรับตามความผันผวนจริง) หรือ % ตายตัว
  FILTERS: ADX>min (เทรนด์แรงพอ), volume>SMA20

ความสัตย์จริง (ตามที่สั่ง — ไม่โกหก/ไม่ปกปิด):
  * ทุกตัวเลขมาจากรันจริง ไม่มี hard-code, ไม่มีเดา.
  * out-of-sample validation ฝังไว้ — overfit ถูกคัดออกอัตโนมัติ.
  * รายงานทั้ง mean และ MEDIAN ของกำไร (median กัน outlier ตัวเดียวหลอก).
...
วิธีใช้บน Colab:
  1) !pip install yfinance pandas numpy --quiet

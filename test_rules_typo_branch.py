...
fixture สะอาดไม่ trigger:
  • r_itm004 — SPELLING_PATTERNS (อังกฤษติดกัน 'WARMWHITE', เลข+หน่วยติดกัน '5นิ้ว', ไทย+อังกฤษติดกัน)
...
# [ADR-075] เลข+หน่วยไทยติดกัน "5นิ้ว" = เขียนไทยปกติ → ต้อง "ไม่ฟ้อง" (ADR-075 ตัด false-positive เดิม)
out2 = B.r_itm004({"items": [_it(1, "ท่อ 5นิ้ว")]}, None, {})
check(out2 == [], "เลข+หน่วยไทยติดกัน (5นิ้ว) → ไม่ฟ้อง (ADR-075 เขียนไทยปกติ)")
# ไทย+อังกฤษ ติดกัน = ของจริงต้องเว้นวรรค → ยังฟ้อง (ครอบ emit branch ของ r_itm004)
out2b = B.r_itm004({"items": [_it(1, "ท่อPVCสีขาว")]}, None, {})
check(len(out2b) > 0, "ไทย+อังกฤษ ติดกัน (ท่อPVCสีขาว) → ฟ้อง")

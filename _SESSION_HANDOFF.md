...
PYTHON=python3 bash run_ci.sh /mnt/project     # รันทั้งชุด (corpus ทางการ = 146 ไฟล์, รันขั้น 7-8)
```

## 1) ค่าคงที่ที่ต้องไม่ขยับ (Golden / Invariants)
- **golden hash จริง (146 ไฟล์ `/mnt/project`) = `814c7cf0`** · **fixture = `72cb832c…`** (เต็ม: `72cb832c9b667e796b86a78ec5e1b0556c9f19b50d15d7756df1cbd0a58e1f0a`)
- คงเส้น: `engine_hash == agent_hash == baseline_hash` เสมอ (ชั้น agent/advisory ห้ามเปลี่ยนผลตรวจหลัก)
- คอร์ปัสทางการ = **146 ไฟล์ (`/mnt/project`)** → golden `814c7cf0…` (engine==agent==baseline). สาย 81 ไฟล์ปลดระวาง (ดู ADR-019).

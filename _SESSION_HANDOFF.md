...
PYTHON=python3 bash run_ci.sh <โฟลเดอร์ 81 ไฟล์จริง>     # รันทั้งชุด (ใส่ path ข้อมูลจริงเพื่อรันขั้น 7-8)
```

## 1) ค่าคงที่ที่ต้องไม่ขยับ (Golden / Invariants)
- **golden hash จริง (81 ไฟล์) = `ec61907f`** · **fixture = `d8bcde85…`** (เต็ม: `d8bcde8555034a203f80d2a596c42ea57b2f1a39ce67003f5629cea03185b07c`)
- คงเส้น: `engine_hash == agent_hash == baseline_hash` เสมอ (ชั้น agent/advisory ห้ามเปลี่ยนผลตรวจหลัก)
- บนชุดทดสอบ sandbox นี้มี 106 ไฟล์ (`/mnt/project`) ไม่ใช่ 81 ทางการ → hash sandbox = `f1ac8421…` (engine==agent) ใช้ยืนยัน "agent ไม่เพี้ยน" เท่านั้น **ตัวเลขทางการต้องรันบน 81 ไฟล์จริง**
...
- **OBJ-0** *(รอผู้ใช้)* — เปลี่ยน VAT002 tolerance `1.00 → 0.50` (ผู้ใช้เลือก 0.50 แล้ว แต่ยังไม่ลงมือ)
  ...
  - **patch** (`rules_engine.py` ใน `r_vat002`):
    ...
    +     if diff < Decimal('0.50'):   # OBJ-0: ตรงกฎ "ยอมต่างปัดเศษ ≤0.50"
              return []
    ```
  - ขั้นตอน: รัน `regression_full.py . <81 ไฟล์>` → ถ้า hash เท่าเดิม จบ ; ถ้าเปลี่ยน(กรณี 0.01) และยอมรับ → `python golden_master.py . tests/fixtures/baseline_fixture.json tests/fixtures` (regen + บันทึกเหตุผล)

...
PYTHON=python3 bash run_ci.sh /mnt/project     # รันทั้งชุด (corpus ทางการ = 106 ไฟล์, รันขั้น 7-8)
```

## 1) ค่าคงที่ที่ต้องไม่ขยับ (Golden / Invariants)
- **golden hash จริง (106 ไฟล์ `/mnt/project`) = `35b2f7c8`** · **fixture = `d8bcde85…`** (เต็ม: `d8bcde8555034a203f80d2a596c42ea57b2f1a39ce67003f5629cea03185b07c`)
- คงเส้น: `engine_hash == agent_hash == baseline_hash` เสมอ (ชั้น agent/advisory ห้ามเปลี่ยนผลตรวจหลัก)
- คอร์ปัสทางการ = **106 ไฟล์ (`/mnt/project`)** → golden `35b2f7c8…` (engine==agent==baseline). สาย 81 ไฟล์ปลดระวาง (ดู ADR-019).

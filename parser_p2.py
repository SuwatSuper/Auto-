...
# [OPT-2 ก] auto re-export parser_p1.__all__ ยกเว้นที่ p2 จัดการเอง (Decimal/ROUND_HALF_UP import ตรงจาก decimal;
#   VAT_RATE/_D มาทาง chain; _RATE_MARKERS/_rightmost_num_has_decimal/_row_has_rate_marker = helper ภายใน p1). golden ไม่ขยับ.
...
def _pb_extract_items(result, block_df, cols):
    ...
    for r in range(len(block_df)):
        ...
        s = str(seq).strip()
        if not _is_seq_token(s): continue          # [H2/L8] กัน superscript (int ระเบิด) + รับ '1.00'
        last_seq = int(float(s))
        if not (1 <= last_seq <= 50): continue
        item = _pb_build_item(block_df, r, last_seq, name_col, qty_col, unit_col, price_col, amt_col)
        if item:
            result['items'].append(item)
...
def _pb_money_col(df, vat_row, amt_col, ncols):
    """[ADR-179/MONEYCOL] คืน "คอลัมน์ยอดเงินท้ายบิล" ที่ใช้อ่าน subtotal/vat/total.

    ROOT CAUSE ที่แก้: `detect_item_columns._dic_find_amt` เลือก amt_col = คอลัมน์ "ขวาสุด"
    ที่มีเลข >100 ในแถวรายการ. เทมเพลตผู้ขายบางเจ้ามีคอลัมน์ช่วย (helper) ทางขวาที่ mirror
    ยอดรายการไว้ + เก็บ "ยอดรวมทั้งไฟล์" ไว้ในแถวท้าย → amt_col ชี้คอลัมน์ helper แทนคอลัมน์เงินจริง
    แล้ว footer scan คว้ายอดรวมทั้งไฟล์มาเป็น subtotal ของบิลเดียว.
      เชิงประจักษ์ (TKH_69_07.xls ชีต 2): amt_col=22(W) → subtotal=134,103.35 (= ยอดรวม 3 ชีต)
      ทั้งที่ยอดจริงของบิล = 89,374.55 (คอลัมน์ 17/R) → VAT001+VAT012 ฟ้องผิดทั้งคู่ (false positive
      ทางการเงิน = ร้ายแรงสุดตามนิยามระบบ) และยอดรวมบริษัทในรีพอร์ตเกินจริง 44,728.80.

    ตัวชี้ขาดที่ใช้ (เชิงโครงสร้าง ไม่ใช่การเดา): **คอลัมน์เงินจริงต้องมีตัวเลขอยู่ใน "แถว VAT"**
    (แถวที่มี rate 0.07) เพราะยอด VAT ถูกพิมพ์คู่กับอัตราเสมอ. คอลัมน์ helper ไม่มี.
      - amt_col มีค่าในแถว VAT → คืน amt_col เดิม (เส้นทางเดิม 100% ของชีตที่ถูกอยู่แล้ว → golden-neutral)
      - ไม่มี → มองหาคอลัมน์ที่มีค่าในแถว VAT (ไล่จากขวาไปซ้าย) → ใช้คอลัมน์นั้นแทน
      - ไม่เจอเลย → คืน amt_col เดิม (fail-safe: พฤติกรรมเดิม ไม่เดา ไม่แต่งค่า)
    ★ ไม่มีการ "ปรับยอดให้ตรง" ใด ๆ — แค่อ่านเซลล์ให้ถูกคอลัมน์. VAT001 ยังจับยอดที่ผิดจริงได้เหมือนเดิม.
    """
    if amt_col is None or vat_row is None:
        return amt_col
    try:
        if amt_col < ncols and _cell_to_num(df.iat[vat_row, amt_col]) is not None:
            return amt_col                      # เส้นทางเดิม — ไม่แตะ
    ...
    for c in range(ncols - 1, -1, -1):
        if c == amt_col:
            continue
        try:
            v = _cell_to_num(df.iat[vat_row, c])
        except (IndexError, KeyError):
            continue
...
def _pb_footer_triple(df, row_start, row_end, ncols):
    """[ADR-182] หา (subtotal, vat, total) ที่ "พิมพ์บนเอกสาร" แบบไม่ต้องพึ่งเซลล์อัตรา 0.07.

    รับเฉพาะชุดที่ **สอดคล้องกันเองทางคณิตศาสตร์**:  vat ≈ subtotal × 7%  และ  total ≈ subtotal + vat
    (tol 0.05 บาท) โดย total ต้องอยู่แถวเดียวกันหรือต่ำกว่าอีกสองตัว. ถ้าเข้าเกณฑ์หลายชุด เลือกชุดที่
    subtotal สูงสุด (= ยอดรวมท้ายบิล ไม่ใช่ยอดรายการกลางบิล). ไม่เข้าเกณฑ์เลย → คืน None (ไม่เดา).

    เหตุที่ปลอดภัย: เงื่อนไข 2 ชั้นพร้อมกัน (7% + ผลบวก) ทำให้จับเลขสุ่มมาเข้าคู่กันแทบเป็นไปไม่ได้ —
    พิสูจน์บน corpus จริง 1085 บิล: ตำแหน่งที่ตรวจเจอตรงกับ footer จริงทุกใบ, ผิดตำแหน่ง 0.
    คืน None เสมอเมื่อไม่มั่นใจ → คงพฤติกรรมเดิม (item_sum) ไม่ regress.
    """
    lo = max(0, row_start)
    hi = min(row_end, df.shape[0] - 1)
    if hi < lo:
        return None
    vals = []
    for r in range(lo, hi + 1):
        for c in range(ncols):
            try:
                f = _cell_to_num(df.iat[r, c])
            except (IndexError, KeyError):
                continue
            if f is not None and f > 1 and math.isfinite(f):
                vals.append((r, f))
    if len(vals) < 3:
        return None
    best = None
    for r_s, sub in vals:
        want_vat = sub * 0.07
        want_tot = sub + want_vat
        cand_v = [(r, f) for r, f in vals if abs(f - want_vat) <= 0.05]
        if not cand_v:
            continue
        cand_t = [(r, f) for r, f in vals if abs(f - want_tot) <= 0.05]
        if not cand_t:
            continue
        r_v, vat = cand_v[0]
        r_t, tot = max(cand_t, key=lambda x: x[0])
        if r_t < r_s or r_t < r_v:          # total ต้องไม่อยู่เหนือ subtotal/vat
            continue
...
def _pb_amounts_from_vatrow(df, result, vat_row, amt_col, row_start, row_end):
    ...
    [ADR-179] อ่านผ่าน _pb_money_col — กัน amt_col ชี้คอลัมน์ helper (ดูเหตุผลเต็มที่ _pb_money_col)"""
    ncols = df.shape[1]
    amt_col = _pb_money_col(df, vat_row, amt_col, ncols)
...
def _pb_finalize_amounts(result):
    ...
    if result['vat'] is None and result['subtotal'] is not None:
        try:
            ...
            if result['total'] is None:
                result['total'] = _money_q(float(result['subtotal']) + result['vat']); src['total'] = 'derived'  # [F-1] HALF_UP
    ...
    if result['subtotal'] is None and result['items']:
        s = _money_q(sum(i['amount'] or 0 for i in result['items']))   # [F-1] ปัด item_sum HALF_UP 2 ตำแหน่ง
        if s and s > 0: result['subtotal'] = s; src['subtotal'] = 'item_sum'
    ...
    if result['subtotal'] is not None:
        try:
            ...
            if result['total'] is None:
                result['total'] = _money_q(_sub + float(result['vat'])); src['total'] = 'derived'  # [F-1] HALF_UP
...
def _filename_period_ce(filename):
    ...
    for mm in re.finditer(r'(?<!\d)(\d{2,4})[.\-_](\d{1,2})', base):
        ...
        if not 1 <= mo <= 12:
            continue
        # [LONGEVITY-FIX] ขยายหน้าต่างปี → ครอบถึง พ.ศ.2599 / ค.ศ.2056 (เดิมหยุด 2582/2039 = parse ชื่อไฟล์ไม่ได้ตั้งแต่ปีไทย 83 = ค.ศ.2040)
        #   golden-safe: ไม่มีไฟล์ในคลังปัจจุบันอยู่ช่วงปี 83-99 → พฤติกรรมไฟล์ปี 67-69 เดิมไม่เปลี่ยน. สอดคล้องกับ _ivp_year2_to_ce / parser_p0a ที่รองรับถึง 2599 อยู่แล้ว
        for lo, hi, adj in ((2558, 2599, -543), (2015, 2056, 0), (58, 99, 1957), (15, 39, 2000)):
            if lo <= y <= hi:
                return (y + adj, mo)
    return None
...
def _pb_prefer_period(df, result, row_start, header_end, ncols, fperiod, sheet_name=None):
    ...
    if not fperiod and sheet_name is not None:
        ...
        if sd.isdecimal():                         # [H2] กัน superscript ในชื่อชีต (.isdigit รับแต่ int ระเบิด)
            day = int(sd)
            for r in range(row_start, header_end):
...
def _parse_block(df, sheet_name, filename, row_start, row_end, block_idx=0):
    ...
    # [ADR-190] คู่ขนาน: address_raw = ที่อยู่ก่อน normalize (ใช้เฉพาะ ADDR011 ตรวจอักขระผิดรูป)
    #   ไม่ถูกใช้เทียบ master/แสดงผล → พฤติกรรมส่วนอื่นไม่กระทบ
    _rawlines = result.pop('_addr_raw_lines', None)
    if _rawlines:
        result['address_raw'] = ' '.join(_rawlines)
    ...
    # 7b) [ADR-182/FOOTER-TRIPLE] อ่าน "ยอดท้ายบิลที่พิมพ์บนเอกสาร" เมื่อ 6/7 หาไม่เจอ
    #   ปัญหาที่แก้: _pb_find_vat_row ต้องเจอเซลล์ที่เป็น "อัตรา 0.07/7%" ก่อนถึงจะอ่าน footer.
    #   เทมเพลตผู้ขายหลายเจ้า (CHM/TSH/SEE/HSH/SBT/TNT…) พิมพ์เฉพาะ "ยอดก่อน VAT / VAT / ยอดรวม"
    #   โดยไม่พิมพ์อัตรา → footer ไม่ถูกอ่านเลย → _pb_finalize_amounts ตกไปใช้ subtotal = ผลรวมรายการ.
    #   ผลเชิงตรรกะ: VAT001 (ผลรวมรายการ ≠ ยอดก่อน VAT) **ฟ้องไม่ได้ตลอดกาล** เพราะสองฝั่ง
    #   เป็นตัวเลขเดียวกันโดยนิยาม → รีพอร์ตขึ้น "ยอดก่อน vat : ตรง" ทั้งที่ไม่เคยเทียบกับเอกสารเลย
    #   = "ตรงหลอก" (ขัดหลักระบบ: ตรวจไม่ได้ ≠ ถูก). เชิงประจักษ์ corpus นี้ 384 บิล (35%) อยู่ในสภาพนี้.
    #   วิธีอ่าน (อนุรักษ์นิยม — รับเฉพาะที่ "สอดคล้องกันเอง" เท่านั้น ไม่เดา):
    #     รับ triple (sub, vat, tot) ก็ต่อเมื่อ vat ≈ sub×7% และ tot ≈ sub+vat และ tot อยู่แถวท้ายสุด
    #     → เกณฑ์เดียวกับ oracle อิสระที่ใช้ยืนยัน 1085 บิล (จับผิดตำแหน่ง 0 ครั้ง).
    #   ★ ไม่แก้ค่าใด ๆ ให้ตรง — แค่ทำให้ "ยอดบนเอกสาร" มีตัวตนเพื่อให้ VAT001 เทียบได้จริง.
    if result['subtotal'] is None:
        _ft = _pb_footer_triple(df, row_start, row_end, ncols)
        if _ft is not None:
            result['subtotal'], result['vat'], result['total'] = _ft

    # 8) reconcile + เติมค่าขั้นสุดท้าย
    _pb_finalize_amounts(result)

    # 8b) [F-MONEYIV v9.3.1] กัน "ยอดเงินบนบิล" ถูกอ่านเป็นเลขที่เอกสาร
    #     ต้องเรียกหลัง finalize_amounts (ต้องมี subtotal/vat/total ครบก่อน
    #     ถึงจะเทียบ iv==ยอดเงิน ได้) ถ้าตรงกัน→ล้าง iv แล้วกู้เลขจริงด้วย lastresort
    #     หมายเหตุ: dormant 100% บน corpus 834 บิล (ไม่มีบิลใด iv==ยอดเงิน) → golden-neutral
    reject_iv_equal_amount(df, result, row_start, header_end, ncols)
...
def _tor_try_date(result, v, s):
    ...
    if d.year >= 2500:
        # [ADR-174/BUGHUNT] twin ของ [M1] puopuy_dates: 29 ก.พ. ของปี พ.ศ.อธิกสุรทิน (เช่น 2568)
        #   ที่ ค.ศ.(ปี-543)=2025 ไม่อธิกสุรทิน → เดิม replace ครัช ValueError ทะลุถึง except ระดับชีต
        #   = บิลทั้งชีต TOR หาย. เส้นวันปกติถูกอุดแล้ว (M1) แต่เส้น TOR นี้ตกหล่น.
        try:
            d = d.replace(year=d.year - 543)   # native datetime พ.ศ. (เส้น string แปลง ค.ศ. แล้ว → ไม่ซ้ำ)
        except ValueError:
            if not result.get('_bad_date'): result['_bad_date'] = str(d)[:20]
            return False
    result['iv_date'] = d
    result['iv_date_str'] = d.strftime('%d/%m/%Y')
    return True

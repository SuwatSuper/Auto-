...
# [ADR-107] 2 คอลัมน์ แต่ qty×price "ไม่ validate แม้แต่แถวเดียว" → คืน (None,None) ไม่เดา
#   (กัน qty=price=amount → ITM001 ฟ้องปลอม V×V บนใบเหมารวม/บริการ). col0=[1,1] col1=[2,5]
#   amount=[100,200]: 1×2=2≠100, 1×5=5≠200 → best_ok=0 → None,None.
qc, pc = P._dic_pick_qty_price(df_q.to_numpy(dtype=object), [0, 1], 3, [(0, [1, 1]), (1, [2, 5])])
check((qc, pc) == (None, None), f"2 คอลัมน์ validate 0 แถว → (None,None) ไม่เดา [ADR-107] (ได้ {qc},{pc})")
# [ADR-107] partial-validate (≥1 แถวตรง แต่ <50%) → ยังใช้ fallback heuristic min/max เดิม
df_p = pd.DataFrame([[2, 50, 0, 100], [1, 7, 0, 200], [1, 9, 0, 300]])
qc2, pc2 = P._dic_pick_qty_price(df_p.to_numpy(dtype=object), [0, 1, 2], 3, [(0, [2, 1, 1]), (1, [50, 7, 9])])
check(qc2 is not None and pc2 is not None,
      f"2 คอลัมน์ partial-validate (1/3) → fallback heuristic ยังเดา ({qc2},{pc2})")

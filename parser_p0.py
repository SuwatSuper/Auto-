...
_IV_LONGDIGIT_RE = re.compile(r'\d{20,}')   # [ADR-174] บล็อกเลขติดกัน ≥20 หลัก = เชื้อ ReDoS (เกินเพดาน IV 13 หลัก)
...
def _pick_best_iv_safe(text):
    ...
    candidates = []
    # [ADR-174/BUGHUNT] กัน ReDoS 2 ชั้น: {0,10} + ตัดบล็อกเลข ≥20 หลัก (สำเนา parser_p1 — เหตุผลเต็มที่นั่น)
    ...
    for cand, pos, raw in candidates:
        score = 0
        # v9.3.2 [FIX-IV-FLOATTAIL] (sync กับ _pick_best_iv): หางทศนิยมของยอดเงิน
        #   (เลข+จุดนำหน้าทันที เช่น 75191.04000000001 → '04000000001') ≠ เลขเอกสาร → หักแรง
        if pos >= 2 and s[pos-1] == '.' and s[pos-2].isdigit() and cand[:1].isdigit():
            score -= 40

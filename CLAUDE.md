...
4. **golden ปัจจุบัน (แหล่งจริง = ไฟล์ ไม่ใช่ค่าที่จำ):**
   - corpus  : `f05358aa…`  (= `baseline.json._sha256`, 152 ไฟล์ `/mnt/project`, 1153 บิล)
...
bills (list of dict)  ──►  rules_engine (~56 กฎ r_*)  ──►  ติด issues[] ให้แต่ละบิล
   │
   ├─►  golden_master/regression_full ──► hash ผลตรวจ = "f05358aa" (oracle)
   │
...
python3 regression_full.py . /mnt/project baseline.json   # 1) golden corpus → engine==agent==baseline=f05358aa
...
```
> ข้อ 1 ไม่ได้ `f05358aa` ตั้งแต่เปิด session = **หยุด แจ้ง Tor ห้ามแก้อะไร** (env/ไฟล์เพี้ยน ต้องสืบก่อน).

---

## 6 · กับดัก (landmines — เคยทำระบบพัง/ข้อมูลหายมาแล้ว · ห้ามแตะ)
- **`pythainlp`: สองสถานะถูกต้อง — ไม่มี หรือ 5.0.5 เป๊ะ** (ADR-164 พิสูจน์ 2026-07-02: venv+5.0.5 จริง → fixture ad0c9dad + real f05358aa เป๊ะเดิม; call-site เดียวคือ webverify นอก golden path) · typo 51 มาจาก `CONSTRUCTION_DICT` · **เวอร์ชันอื่นห้าม** (gate FAIL ที่ major/unknown).
...
- แยก 2 เคสก่อน (forensic-first): **(A) parser ล่ม/อ่านเซลล์ผิด** → harden กันล่ม; ถ้า **golden 152 ไฟล์เท่าเดิม** = **golden-neutral แก้ได้ (พิสูจน์ + 1 ADR)** ← ช่องที่ทำให้ดักบั๊ก input ไม่ตัน. **(B) กฎควรจับ/ไม่ควรจับเพิ่ม** → เปลี่ยนผลตรวจ = **STOP-AND-ASK**.
- บังคับพิสูจน์: หลังเพิ่ม guard รัน `golden_master` → corpus **ต้องได้ `f05358aa` เป๊ะ**. กลไก/แพตเทิร์น: `parser_guards.py` · `file_guard.py` · `log_system_issue()` (กฎ crash → ชีต System Issues ไม่ปนผล) · `_failed_files` (ข้ามไฟล์เสีย ที่เหลือยังครบ) · money-serial guard (ADR-055) · DOC001 guard (ADR-057/058). เทส: `test_input_hardening.py`, `test_parser_negative.py`, `test_bughunt_hardening.py`.
...
2. ยืนยัน baseline (ข้อ 5.0–5.1): ล้าง pycache → `regression_full` → **ต้องได้ `f05358aa`**. ไม่ได้ = **หยุด แจ้ง Tor**.

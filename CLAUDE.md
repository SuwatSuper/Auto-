...
4. **golden ปัจจุบัน (แหล่งจริง = ไฟล์ ไม่ใช่ค่าที่จำ):**
   - corpus  : `08e6abfd…`  (= `baseline.json._sha256`, 148 ไฟล์ `/mnt/project`, 1056 บิล)
...
bills (list of dict)  ──►  rules_engine (~56 กฎ r_*)  ──►  ติด issues[] ให้แต่ละบิล
   │
   ├─►  golden_master/regression_full ──► hash ผลตรวจ = "08e6abfd" (oracle)
   │
...
python3 regression_full.py . /mnt/project baseline.json   # 1) golden corpus → engine==agent==baseline=08e6abfd
...
```
> ข้อ 1 ไม่ได้ `08e6abfd` ตั้งแต่เปิด session = **หยุด แจ้ง Tor ห้ามแก้อะไร** (env/ไฟล์เพี้ยน ต้องสืบก่อน).
...
- บังคับพิสูจน์: หลังเพิ่ม guard รัน `golden_master` → corpus **ต้องได้ `08e6abfd` เป๊ะ**. กลไก/แพตเทิร์น: `parser_guards.py` · `file_guard.py` · `log_system_issue()` (กฎ crash → ชีต System Issues ไม่ปนผล) · `_failed_files` (ข้ามไฟล์เสีย ที่เหลือยังครบ) · money-serial guard (ADR-055) · DOC001 guard (ADR-057/058). เทส: `test_input_hardening.py`, `test_parser_negative.py`, `test_bughunt_hardening.py`.
...
2. ยืนยัน baseline (ข้อ 5.0–5.1): ล้าง pycache → `regression_full` → **ต้องได้ `08e6abfd`**. ไม่ได้ = **หยุด แจ้ง Tor**.

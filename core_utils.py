...
def _df_safe(df: Any) -> Any:
    ...
    def _c(v: Any) -> Any:
        if isinstance(v, str):
            v = _XL_ILLEGAL.sub('', v)
            # [ADR-176] กัน formula injection ('=' นำหน้า) — corpus ปัจจุบัน 0 เคส → determinism ไม่ขยับ
...
def _bill_date_key(d: object) -> tuple[int, int, int, int]:
    """[ADR-173/BUGHUNT] คีย์เรียงวันที่กันชนิดผิด: date/datetime ปกติ → (0,y,m,d) ลำดับเดิมเป๊ะ ;
    None/str/serial/NaT → (1,…) = ท้ายสุด (semantics เดียวกับ `or datetime.max` เดิม). เดิม iv_date
    เป็น str (บิลเพี้ยน) ทำ '<' str vs datetime → TypeError ล้ม export_excel ทั้งไฟล์ (full mode).
    [ADR-188] เติม annotation — ไฟล์นี้อยู่ชั้น leaf ที่ `test_typing_leaf.py` บังคับ mypy strict.
    `d: object` เพราะรับได้ทุกชนิดโดยเจตนา (นั่นคือหน้าที่ของฟังก์ชัน) ; พฤติกรรมไม่เปลี่ยนแม้แต่บิตเดียว."""
    try:
        _y: Any = getattr(d, 'year', None)
        _m: Any = getattr(d, 'month', None)
        _d: Any = getattr(d, 'day', None)
        return (0, int(_y), int(_m), int(_d))   # None/str ขยะ → TypeError/ValueError ตกไป except (เจตนาเดิม)
...
def sort_bills_by_date(bills: list[dict]) -> list[dict]:
    ...
    # [ADR-173] คีย์วันที่เปลี่ยนเป็น _bill_date_key — date ปกติลำดับเดิมเป๊ะ, ชนิดผิดไปท้ายแทนที่จะครัช.
    return sorted(bills, key=lambda b: (_bill_date_key(b.get('iv_date')),

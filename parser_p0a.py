...
def merge_continuation_bills(bills):
    ...
    for key in order:
        ...
        if len(used_sheets) > 1:
            ...
            # [ADR-183/MERGE-FOOTER] ยอดที่ "พิมพ์บนเอกสาร" ต้องชนะยอดที่ระบบคำนวณเอง.
            #   เดิม: merge แล้วคำนวณ subtotal/vat/total ใหม่จาก qty×price ทับเสมอ → ยอดท้ายบิลที่
            #   พิมพ์ไว้จริงบนหน้าสุดท้ายถูกทิ้ง. ผล: (1) VAT001 (ผลรวมรายการ ≠ ยอดก่อน VAT) ฟ้องไม่ได้
            #   เพราะสองฝั่งกลายเป็นตัวเลขเดียวกัน = "ตรงหลอก"  (2) ส่วนต่างปัดเศษที่ผู้ขายพิมพ์ผิดจริง
            #   หายไปเงียบ — เชิงประจักษ์ TKH_69_0462 ชีต "4 + 4 (2)": เอกสารพิมพ์ VAT 107,116.52 /
            #   รวม 1,637,352.24 แต่ระบบเขียนทับเป็น 107,116.50 / 1,637,352.22 (ต่าง 0.02) โดยไม่แจ้ง.
            #   ใหม่: ถ้าหน้าใดในชุดมียอด "อ่านจากเอกสารจริง" (provenance='ocr') ให้ใช้ของหน้าหลังสุด
            #   (หน้าที่มี footer ท้ายบิล) แทน แล้วปล่อยให้ VAT001 เป็นผู้ตัดสินว่าตรงกับรายการหรือไม่.
            _printed = None
            for _pg in grp_sorted:
                if str(_pg.get('sheet', '')) not in used_sheets:
                    continue
                if (_pg.get('amount_source') or {}).get('subtotal') == 'ocr' and _pg.get('subtotal') is not None:
                    _printed = _pg
            if _printed is not None:
                new_bill['subtotal'] = _printed.get('subtotal')
                new_bill['vat'] = _printed.get('vat')
                new_bill['total'] = _printed.get('total')
                new_bill['amount_source'] = dict(_printed.get('amount_source') or {})
                if _printed.get('total_text') and not new_bill.get('total_text'):
                    new_bill['total_text'] = _printed.get('total_text')
                _sub = 0.0                      # ข้ามการเขียนทับด้วยค่าคำนวณด้านล่าง
            else:
                # ไม่มีหน้าใดพิมพ์ยอดไว้ → คำนวณ subtotal จาก items ที่ครบ (qty×price) ตามเดิม
                _sub = 0.0

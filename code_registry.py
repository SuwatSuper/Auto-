...
# [ADR-141 แก้ ADR-127] เอา ADDR001 ออก — r_addr001 มี "standalone path" (rules_a:248-257):
#   address ว่าง→'ไม่พบที่อยู่' ; ไม่มีไปรษณีย์ 5 หลัก/ไม่มีจังหวัด-เขต-แขวง → ฟ้อง "แม้ไม่มี master"
#   (เทียบ master เป็นแค่ branch เพิ่ม). ADDR001 จึง "active จริง" ไม่ใช่ dormant-เพราะ-ไม่มี-master.
#   (corpus=0 เพราะที่อยู่ครบ ไม่ใช่เพราะ master ว่าง). ADDR002/003 มี `if not m: return []` บรรทัดแรก = master-dependent จริง.
MASTER_DEPENDENT = {

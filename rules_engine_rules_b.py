...
def r_itm013(b,m,c):
    ...
    # [BUGFIX recheck] รับเฉพาะ seq ที่เป็น int แท้ (parser จริงผลิต int เสมอ) — กัน str '1'!=1 → false positive
    seqs = [i['seq'] for i in items if isinstance(i.get('seq'), int) and not isinstance(i.get('seq'), bool)]
...
def r_itm014(b,m,c):
    ...
    # [L5] รับเฉพาะ seq int แท้เหมือน r_itm013 — เดิม sorted([... None/str ...]) → TypeError → ข้ามทั้งกฎเงียบ
    seqs = sorted({i['seq'] for i in items
                   if isinstance(i.get('seq'), int) and not isinstance(i.get('seq'), bool)})
    if len(seqs) < 2: return []
    o = []
    gaps = []
    for i in range(len(seqs)-1):
...
def r_itm011(b,m,c):
    ...
    for it in b['items']:
        ...
        # v9.3.5 [ADR-119]: กัน FP จากการ "ตัดคำกลางคำ" ของ regex เดิม `[ก-๙][ก-๙์]{3,19}`
        #   regex เดิมจำกัด 20 ตัว/คำ → คำประสมไทยยาวเกิน 20 (ไม่มีช่องว่าง เช่น
        #   'ข้อต่อสามทางเกลียวชุบสังกะสี' = 28 ตัว) ถูก findall (greedy, non-overlapping) ตัดเป็น
        #   หน้าต่าง 20 ตัว → ท่อนหลังท่อนแรกเริ่มกลางคำ = เศษคำ ('...ชุ' | 'บสังกะสี') แล้ว 'บสังกะสี'
        #   fuzzy ใกล้ 'สังกะสี' 93% = false-positive (ต้นฉบับ 'ชุบสังกะสี' ถูก 100%).
        #   แก้: ดึง "Thai run" ทั้งก้อนก่อน แล้วเก็บ "เฉพาะท่อนแรก" ของแต่ละ run (run[:20]) —
        #   ขอบซ้ายของท่อนแรก = ขอบ run จริงเสมอ (เท่ากฎเดิมเป๊ะ) ; เศษ (ท่อนที่ 2+) จากการตัด run
        #   ยาวเป็น fragment ขอบซ้ายปลอม → ไม่น่าเชื่อถือ → ข้าม (ไม่กระทบคำ ≤20 ตัวเดิมแม้แต่ตัวเดียว).
...
def r_vat004(b,m,c):
    ...
    for f, v in [('subtotal',b.get('subtotal')),('vat',b.get('vat')),('total',b.get('total'))]:

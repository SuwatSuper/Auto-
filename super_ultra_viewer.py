...
# [ADR-185/NO-HIDE] เดิม: _HIDE_IN_SUMMARY = {"ITM015","ITM018","ITM016"} ตัด 3 รหัสนี้ทิ้งจาก
#   company_summary.txt ทั้งหมด → ช่อง "รายการสินค้า" ขึ้นคำว่า "ตรง" และบรรทัดท้ายขึ้น
#   "ที่เหลือตรงครับผม" ทั้งที่ยังมีจุดฟ้องค้างอยู่ (เชิงประจักษ์: ultra .txt มี ITM015 254 ครั้ง /
#   company_summary.txt มี 0). = ขัดหลักของระบบเอง "ตรวจไม่ได้ ≠ ถูก" และขัดคำสั่งเจ้าของระบบ
#   ("อะไรผิด ก็ต้องบอกผิด").
#   ใหม่: ไม่ซ่อนรหัสใดทั้งสิ้น. false positive ต้องแก้ที่ "ต้นเหตุของกฎ" (เช่น ADR-184 ยุบหน่วย
#   Pcs.↔ท่อน ตัด FP 154 จุด) ไม่ใช่ปิดปากรีพอร์ตปลายทาง.
_HIDE_IN_SUMMARY: set = set()
_XLSX_ONLY: set = set()      # ทุกจุดขึ้นทั้ง .txt และ Excel worklist (ไม่มีจุดที่เห็นได้ที่เดียว)
...
def _sv_num(v):
    """[ADR-173/BUGHUNT] ยอดเงินปลอดภัยสำหรับผลรวม viewer — twin ของ reporting_p1._fin ([REP-C2]):
    None/str/NaN/±inf → 0.0 ; float ปกติค่าเท่าเดิม (company_summary byte-identical บนบิลปกติ)."""
    try:
        f = float(v)
...
def build(bills, master_present=True, masters=None):
    ...
    for _b in bills:
        ...
        _items_ok = isinstance(_it, list) and all(isinstance(_x, dict) for _x in _it)
        # [ADR-173/BUGHUNT] ขยาย gate ระดับบิล (twin ADR-168/172 ฝั่ง viewer): เดิมครัชทั้ง build →
        #   company_summary .txt+.xlsx หายทั้งชุด — iv_date ชนิดผิด (.year ที่ _month_label) / NaT
        #   (format nan) / company non-str (.strip ที่ _norm_company) / issues non-list (for i in …).
        #   บิลปกติผ่านทุกเงื่อนไข → append object เดิม → byte-identical (สัญญาเดิมของ ADR-153).
        if isinstance(_b, dict):
            _ivd = _b.get("iv_date")
            _ivd_ok = _ivd is None or (isinstance(getattr(_ivd, "year", None), int)
                                       and isinstance(getattr(_ivd, "month", None), int))
            _comp_ok = all(isinstance(_b.get(_k), (str, type(None))) for _k in ("company", "company_raw"))
            _iss_ok = isinstance(_b.get("issues", []), list)
        else:
            _ivd_ok = _comp_ok = _iss_ok = False
        if isinstance(_b, dict) and _items_ok and _ivd_ok and _comp_ok and _iss_ok:
            _norm.append(_b)
        else:
            _c = dict(_b) if isinstance(_b, dict) else {}
            _c["items"] = [_x for _x in _it if isinstance(_x, dict)] if isinstance(_it, list) else []
            if not _ivd_ok: _c["iv_date"] = None
            for _k in ("company", "company_raw"):
                _v = _c.get(_k)
                if _v is not None and not isinstance(_v, str): _c[_k] = str(_v)
            if not isinstance(_c.get("issues", []), list): _c["issues"] = []
            _norm.append(_c)
    bills = _norm
    ...
    for _gkey, gbills in groups.items():
        ...
        # ยอด/บิล
        # [ADR-173/BUGHUNT] _sv_num แทน float ดิบ (twin [REP-C2]): เงินเป็น str ('1,234.00') เดิมครัช
        #   ทั้ง build ; NaN เป็น truthy → ยอดกลุ่มกลายเป็น "nan บาท" ในไฟล์ส่งลูกค้า. float ปกติค่าเดิมเป๊ะ.
        total_sum = sum(_sv_num(b.get("total")) for b in gbills)
        # [L3] subtotal==0 (จริง) ไม่ควรตกไป fallback — เดิมใช้ `or` ทำให้ 0 ถูกแทนด้วย total-vat
        prevat_sum = sum(
            (_sv_num(b["subtotal"]) if b.get("subtotal") is not None
             else (_sv_num(b.get("total")) - _sv_num(b.get("vat"))))
            for b in gbills)
        nbills = len(gbills)
        ...
        for b in gbills:
            ...
            for i in b.get("issues", []):
                ...
                _entry = {
                    ...
                    "iv": b.get("iv_number_raw") or b.get("iv_number", "") or "",  # [DISPLAY-FAITHFUL] เลขดิบตามใบ (มีขีด)
...
def _pinpoint_field(field, entries):
    ...
    if field in (F_NAME, F_BRANCH):   # [ADR-099] ชื่อบจ./สาขา: ยุบตาม "ข้อความไม่ตรงที่ไม่ซ้ำ" (เหมือนทุกใบ→1 บรรทัด ; เดิม list ทุกใบ ยาว ~4,900 ตัวอักษร ดูเหมือนเครื่องดัมพ์)
        seen = []
        for fx in (entries or []):
            d = re.sub(r"\s*\(คาดว่า[^)]*\)", "", (fx.get("detail") or fx.get("type") or "").split("| สินค้า")[0]).strip()
            if d and d not in seen: seen.append(d)
        return (" ; ".join(seen) if seen else "ไม่ตรง ภ.พ.20") + " รีเช็คครับ"
    parts = []
    # [ADR-191] ใบซ้ำข้ามไฟล์ (DOC003) — ยุบเป็นบรรทัดเดียวต่อคู่ไฟล์ พร้อม "ยอดรวมที่ถูกนับซ้ำ"
    #   ถ้า list ทีละใบ จะได้ 20+ ท่อนในบรรทัดเดียว อ่านไม่ไหวและกลบสาระ. สิ่งที่บัญชีต้องรู้คือ
    #   "ไฟล์ไหนซ้ำกับไฟล์ไหน กี่ใบ รวมกี่บาท" แล้วไปตัดไฟล์ทิ้งหนึ่งอัน — ไม่ใช่ไล่ทีละใบ.
    _dupe = [fx for fx in (entries or []) if fx.get("code") == "DOC003"
             and "ถูกนับซ้ำ" in (fx.get("detail") or "")]
    if _dupe and len(_dupe) == len(entries or []):
        from collections import defaultdict as _ddx
        _bypair = _ddx(list)
        for fx in _dupe:
            _m = re.search(r"มีอยู่ในไฟล์ (\S+) ด้วย", fx.get("detail") or "")
            _a = re.search(r"ยอด ([\d,]+\.\d{2}) ถูกนับซ้ำ", fx.get("detail") or "")
            _bypair[(fx.get("prefix", ""), _m.group(1) if _m else "?")].append(
                float(_a.group(1).replace(",", "")) if _a else 0.0)
        _segs = []
        for (_pfx, _other), _amts in sorted(_bypair.items()):
            _segs.append(f"ไฟล์ {_pfx} มีใบซ้ำกับไฟล์ {_other} จำนวน {len(_amts)} ใบ "
                         f"รวม {sum(_amts):,.2f} บาทที่ถูกนับซ้ำ — ต้องเลือกใช้ไฟล์เดียว")
        return " ; ".join(_segs) + " รีเช็คครับ"
    for fx in entries:
    ...
    if field == F_ITEM:
        ...
        # [ADR-108/UX] ITM020 (ทั้งบิลไม่มีหน่วยสินค้า) เกิดซ้ำหลายสิบบิล/ไฟล์ → "ยุบเป็นสรุปต่อไฟล์"
        #   1 บรรทัด/ไฟล์ (นับจำนวนบิล) ไม่ร่ายทีละบิล (เดิมยาว 50 บรรทัด ดูเหมือนเครื่องดัมพ์) ;
        #   typo รายการสินค้าจริง (ITM004/005/010/011/019 มี seq) คงชี้ทีละตัวให้เปิดไปแก้ถูกจุด.
        itm020_cnt = {}                      # prefix -> count (dict รักษาลำดับไฟล์ตามที่พบ)
        typo = []                            # (fx, core) ที่ไม่ใช่ ITM020 (ชี้รายตัวตามเดิม)
        for fx, core in zip(entries, parts):
            if fx.get("code") == "ITM020":
                pre = fx.get("prefix", "")
        ...
        if cur:
            lines.append(", ".join(cur))
        for pre, cnt in itm020_cnt.items():
            lines.append(f"ไฟล์ {pre}: {cnt} บิลไม่มีหน่วยสินค้าทั้งบิล (ช่องหน่วยว่างในต้นฉบับ)" if pre
...
def render_block(n, r):
    ...
    # [ADR-186/NO-SOFT-LANE] เดิมแบ่งจุดเป็น 2 ชั้น: clear → บรรทัดช่องหลัก, soft → บรรทัดแยก
    #   "ตรวจตาเพิ่ม (ช่อง) : … — ก้ำกึ่ง ขอตาคนยืนยันครับ" และ **ช่องหลักขึ้นคำว่า "ตรง"**.
    #   ผลจริง: บิลที่มีแต่จุด soft → ผู้ใช้เห็น "ตรง" ในช่อง แล้วสรุปท้ายขึ้น "ตรงเท่าที่ตรวจได้"
    #   = รีพอร์ตขัดกันเองในบล็อกเดียว. เจ้าของระบบสั่งชัด: "ไม่ต้องตรวจตาเพิ่ม — อะไรผิดก็ต้องบอกผิด
    #   ตามฟอร์ม". ใหม่: ทุกจุดขึ้นบรรทัดช่องตัวเองตามฟอร์มเดียวกันหมด ไม่มีชั้นก้ำกึ่ง ไม่มีคำว่า "ตรง"
    #   ทับจุดที่ยังฟ้องค้างอยู่. (คุมความแม่นด้วยการแก้กฎที่ต้นเหตุแทน — ดู ADR-184)
    for f in FIELD_ORDER:
        if f in (F_PREVAT, F_POSTVAT):
            continue
        if f in by_field:                              # มีจุดฟ้อง → ระบุจุดตามฟอร์ม
    ...
    # ★ บรรทัดสรุปท้าย — ช่องที่ยังมีปัญหาทั้งหมด (ไม่มีการยกเว้นช่องใดออกอีกแล้ว)
    footer_problems = [f for f in FIELD_ORDER if f in (r["fix"] + r["check"])]

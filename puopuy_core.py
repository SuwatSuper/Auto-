...
def has_hidden_chars(s: Any) -> list[str]:
    ...
    [ADR-190] + สระอำผิดรูป: `normalize_text` แปลง นิคหิต(U+0E4D)+สระอา(U+0E32) → สระอำ(U+0E33)
      เงียบ ๆ เพื่อให้ match ข้อความได้ (จำเป็น ห้ามถอด) — แต่ "เงียบ" ทำให้ผู้ใช้ไม่เคยรู้ว่า
      ต้นฉบับสะกดผิดอักขระ. เชิงประจักษ์ corpus 146 ไฟล์: 57 เซลล์ใน 4 ไฟล์ เช่น
      'น้ําปลา' (ควรเป็น น้ำปลา) · 'ไม้อัดเคลือบฟิล์มดํา' (บรรทัดข้างบนในบิลเดียวกันสะกด 'ดำ' ถูก)
      · 'สํานักงานใหญ่' · 'อําเภอบางบัวทอง'. เรนเดอร์เหมือนกันแต่คนละ byte → ถ้าส่งออกไปจับคู่
      กับระบบอื่น/ค้นหา จะไม่ตรงและหาไม่เจอ. → ต้องแจ้ง (normalize ต่อไปเพื่อ match, แต่ไม่เงียบ)"""
    if not s: return []
    s = str(s); out = []
    ...
    if '\u0e4d\u0e32' in unicodedata.normalize('NFC', s):
        out.append('สระอำเขียนผิดรูป (นิคหิต ◌ํ + สระอา า แทน ำ — ตาเห็นเหมือนกัน แต่คนละอักขระ)')
    return out

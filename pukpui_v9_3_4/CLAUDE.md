...
4. **golden ปัจจุบัน (แหล่งจริง = ไฟล์ ไม่ใช่ค่าที่จำ):**
   - corpus  : `ae84d3f0…`  (= `baseline.json._sha256`, 148 ไฟล์ `/mnt/project`, 1056 บิล)
...
bills (list of dict)  ──►  rules_engine (~56 กฎ r_*)  ──►  ติด issues[] ให้แต่ละบิล
   │
   ├─►  golden_master/regression_full ──► hash ผลตรวจ = "ae84d3f0" (oracle)
   │
...
python3 regression_full.py . /mnt/project baseline.json   # 1) golden corpus → engine==agent==baseline=ae84d3f0
...
```
> ข้อ 1 ไม่ได้ `ae84d3f0` ตั้งแต่เปิด session = **หยุด แจ้ง Tor ห้ามแก้อะไร** (env/ไฟล์เพี้ยน ต้องสืบก่อน).
...
2. ยืนยัน baseline (ข้อ 5.0–5.1): ล้าง pycache → `regression_full` → **ต้องได้ `ae84d3f0`**. ไม่ได้ = **หยุด แจ้ง Tor**.

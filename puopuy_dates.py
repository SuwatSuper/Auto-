...
def parse_date_any(v: Any) -> datetime | None:
    ...
    if isinstance(v, (int, float)):
        ...
        # [ADR-053 DT-FIX2] serial ที่เก็บ "ปี พ.ศ. ตรงๆ" (เช่น 244471 = พ.ศ.2569) เกินช่วง CE 30000-70000
        #   → แปลงผ่าน xldate แล้วลบ 543 (สมมาตรกับสาขา datetime บรรทัด 32-33). กันไฟล์/read path ที่ส่ง
        ...
        if xlrd is not None and 200000 < v < 300000:
            def _be_serial() -> "datetime | None":
                ...
                return d if 2015 <= d.year <= 2056 else None
            return safe(_be_serial)
        return None
    s = str(v).strip()
    ...
    # [P4] อนุญาต label นำหน้าในเซลล์เดียว ('วันที่ 11/05/69', 'Date: 5/5/69') — ใช้ re.search แทน ^…$
    #   ให้สอดคล้องสาขาเดือนไทยด้านบน (ที่ใช้ re.search อยู่แล้ว). เดิม anchor ^ ตัดทิ้ง → ตก strptime → None.
    m_yy = re.search(r'(?<!\d)(\d{1,2})[/\-.](\d{1,2})[/\-.](\d{2})(?:[ T]\d{1,2}:\d{2}(?::\d{2})?)?(?!\d)', s)
    if m_yy:
        try:
            ...
            # [DT-FIX ADR-052] ปี 2 หลักนอกช่วงปีจริง (เช่น "13" ที่ re.search จับจากเลขที่อยู่
            #   "2/12-2/13" → "12/2/13") = "ตีความไม่ได้" → ห้าม fabricate วันที่. เดิม fallback
            #   (yy+2500)-543 ให้ 1970 ผิด (ที่อยู่ถูกอ่านเป็นวันที่ → ชนะ date cell จริง → false flag).
            #   เคารพ contract _ivp_year2_to_ce: ช่วงกำกวมคืน None ให้ชั้นบนตัดสิน (ดู docstring มัน).
            #   corpus 66–69 อยู่ในช่วง 58–99 → _ce ไม่เป็น None → พฤติกรรมเดิมเป๊ะ (golden ไม่ขยับ).

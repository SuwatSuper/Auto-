...
# [ADR-162] CLI-only guard: ไฟล์นี้รัน logic ที่ module-level (subprocess ทั้ง suite + อ่าน .coverage.json)
# — import โดยตรงจะเกิด side-effect เงียบ/crash. ทุกผู้ใช้จริงรันเป็น script (run_ci [10]) /
# อ่านเป็น text+AST (test_adr138). กันพลาดถาวร: import → fail เร็ว+ชัด แทน side-effect เงียบ.
if __name__ != "__main__":
    raise RuntimeError(
        "coverage_gate.py เป็น CLI dev-tool (module-level side-effect) — "
        "ห้าม import ; รันเป็นสคริปต์: python3 coverage_gate.py"
    )
...
TESTS = [
    ...
    # [ADR-179/179/181] ครอบ branch ของสายอ่าน "ยอดเงินบนเอกสาร" ที่เพิ่มใหม่ใน parser_p2
    #   (_pb_money_col / _pb_footer_triple) — ถ้าไม่ใส่ branch parser family จะตกต่ำกว่าเกต
    ["test_adr179_moneycol_footer.py"],
...
except (FileNotFoundError, json.JSONDecodeError) as _e:  # [ADR-162] fail ชัด แทน traceback
    print(f"❌ อ่าน .coverage.json ไม่ได้ ({type(_e).__name__}) — ติดตั้ง coverage ก่อน: "
          f"pip install coverage --break-system-packages -c constraints.txt")
    sys.exit(2)

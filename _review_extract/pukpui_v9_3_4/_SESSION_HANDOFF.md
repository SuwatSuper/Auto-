...
PYTHON=python3 bash run_ci.sh /mnt/project     # รันทั้งชุด (corpus ทางการ = 148 ไฟล์, รันขั้น 7-8)
```

## 1) ค่าคงที่ที่ต้องไม่ขยับ (Golden / Invariants)
- **golden hash จริง (148 ไฟล์ `/mnt/project`) = `ae84d3f0`** · **fixture = `b5c415bb…`** (เต็ม: `b5c415bbd7bf58bac4328fec1c868325e0955f423d01e9695ba50015fc2f02eb`)
- คงเส้น: `engine_hash == agent_hash == baseline_hash` เสมอ (ชั้น agent/advisory ห้ามเปลี่ยนผลตรวจหลัก)
- คอร์ปัสทางการ = **148 ไฟล์ (`/mnt/project`)** → golden `ae84d3f0…` (engine==agent==baseline). สาย 81 ไฟล์ปลดระวาง (ดู ADR-019).

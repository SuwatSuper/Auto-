...
"""
import os, sys, json, glob, warnings, io, contextlib, argparse
warnings.filterwarnings('ignore')

# [v9.3 STEP7 — P3-A] argparse แบบ backward-compatible: positional เดิม (pkg baseline data)
#   ใช้ได้เป๊ะเหมือนเดิม (regression_full.py / run_ci.sh เรียกแบบนี้อยู่) + เพิ่ม --baseline/--data
#   กันสับ arg กับ golden_master (ที่ตัวที่ 2 = output ไม่ใช่ baseline) ; flag ชนะ positional.
_ap = argparse.ArgumentParser(
    description='ตรวจ golden ฝั่ง agent: รัน pipeline ผ่าน agents.orchestrator แล้วเทียบ baseline')
_ap.add_argument('pkg', nargs='?', default='.', help='โฟลเดอร์ระบบ (default: .)')
_ap.add_argument('baseline_pos', nargs='?', default=None, metavar='baseline',
                 help='ไฟล์ baseline (positional เดิม — default: baseline.json)')
_ap.add_argument('data_pos', nargs='?', default=None, metavar='data',
                 help='โฟลเดอร์ข้อมูล (positional เดิม — default: /mnt/project)')
_ap.add_argument('--baseline', default=None, help='ไฟล์ baseline (flag — ชนะ positional)')
_ap.add_argument('--data', default=None, help='โฟลเดอร์ข้อมูล (flag — ชนะ positional)')
_args = _ap.parse_args()
PKG_DIR = _args.pkg
BASELINE = _args.baseline if _args.baseline is not None else (_args.baseline_pos or 'baseline.json')
DATA = _args.data if _args.data is not None else (_args.data_pos or '/mnt/project')
print(f"[verify_golden] pkg={PKG_DIR} baseline={BASELINE} data={DATA}", file=sys.stderr)

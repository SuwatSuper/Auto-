...
- **D1 (IV007):** จับเลขขยะที่ IV002 (consistency-only) ปล่อยหลุด — ศูนย์ล้วน / เลขเดียวซ้ำ / placeholder
  ('0000000002') / ตรงเศษทศนิยมของยอดเงิน. ไม่พึ่ง master/บิลอื่น. conservative.
- **D2 (parser guard):** ตัดต้นตอ — `_pb_try_iv` ปฏิเสธ candidate ที่ `iv_digits_garbage()` จับ (single-source
  ใน `core_utils` ใช้ร่วม D1). **bounded:** เปลี่ยน iv เฉพาะบิลที่ iv เป็นเลขขยะ (ชุดเดียวกับที่ IV007 ฟ้อง) —
  ไม่ broad. ยังไม่ขยายจับเลขไม่ขึ้นต้น 'IV' (เลี่ยง golden กว้าง — เสนอรอบถัดไป).
- **D3 (DEFERRED):** sanitize ค่าเงินดิบ (1416233.0000000002 → 1416233.00) — **ยังไม่ทำ** เพราะเสี่ยง golden
  สูง+กว้าง (กระทบ VAT001/002/003 ทั่ว corpus), ใกล้โซน ⛔ "ห้ามแตะยอด VAT/total/subtotal", และคุณค่า
  ส่วนเพิ่มต่ำ (บั๊กจริง iv ขยะ แก้ครบด้วย D1+D2 แล้ว). รายละเอียดข้อเสนอใน ADR-033 — รอเจ้าของอนุมัติ.

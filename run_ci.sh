...
#   bash run_ci.sh /path/to/data   # + regression เต็มบนข้อมูลจริง (baseline f05358aa…) ถ้าระบุ data dir
#
...
run "[3x12e] branch-aware master key (ADR-151 — HQ+สาขาอยู่ร่วมกัน → tax-join disambiguate ครบวง)" "$PY" test_master_key_branch_adr151.py
...
if [ -n "$REAL_DATA" ]; then
  # [7pre] [ADR-158] แยก "data drift" (rebaseline ปกติ) ออกจาก "code drift" (bug) ก่อนเทียบ golden —
  #   ถ้า corpus เปลี่ยน tool จะบอกชัด + ให้ recipe rebaseline แทนที่จะเห็น golden-mismatch ลึกลับใน [7]
  run "[7pre] corpus manifest (แยก data-drift ↔ code-drift · ADR-158)" \
      "$PY" verify_corpus_manifest.py "$REAL_DATA"
  run "[7] regression (ข้อมูลจริง 152 ไฟล์, baseline f05358aa…)" \

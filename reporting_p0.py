...
def generate_visual_dashboard(all_bills, summary, cat_counts):
    try:
        ...
        sev_counts = {sev: sum(1 for b in all_bills for i in b['issues'] if i['severity']==sev) for sev in ['CRITICAL','ERROR','WARNING','INFO']}
        ...
        comp = sorted([(s['key'], sum(len(b['issues']) for b in s['bills'])) for s in summary], key=lambda x: -x[1])
...
def field_status(bills, codes):
    affected = sum(1 for b in bills if any(i['code'] in codes for i in b['issues']))
...
def display_low_confidence_bills(all_bills):
    ...
    for b in low:
        rows.append({
            ...
            'บริษัท': (b.get('company') or '')[:40],
            'เหตุผลที่ confidence ต่ำ': ' | '.join(b.get('parse_confidence_reasons', [])),
        })
...
def display_executive_dashboard(all_bills, summary, iv_issues, typos, filename_issues, file_count):
    ...
    clean = sum(1 for b in all_bills if not b['issues'])
    stats = {'files':file_count,'bills':len(all_bills),'clean':clean,
             ...
             'subtotal':sum(b['subtotal'] or 0 for b in all_bills),
             'vat':sum(b['vat'] or 0 for b in all_bills),
             'total':sum(b['total'] or 0 for b in all_bills)}
    pn_badge = ' • PyThaiNLP ✅' if PYTHAINLP_AVAILABLE else ''
    display(HTML(f'''<div style="background:#0F172A;color:#FAFAF9;padding:40px;border-radius:12px;margin-bottom:24px;box-shadow:0 4px 6px rgba(0,0,0,0.04),0 10px 15px rgba(0,0,0,0.06);font-family:'Inter','Noto Sans Thai',-apple-system,sans-serif;"><h1 style="margin:0;font-size:26px;font-weight:600;letter-spacing:-0.02em;">Invoice Audit Report</h1><p style="margin:10px 0 0 0;font-size:12px;opacity:0.55;font-weight:400;letter-spacing:0.02em;">v5.8 · {datetime.now().strftime("%d/%m/%Y %H:%M")} · {len(RULES)} Rules{pn_badge}</p></div>'''))
    ...
    hr = [b for b in all_bills if any(i['severity']=='CRITICAL' for i in b['issues'])]
    if hr:
        rows = []
        for b in hr[:30]:
            rows.append({'ไฟล์':b['file'],'ชีต':b['sheet'],'IV':b['iv_number'],
                'วันที่':b['iv_date_str'],'บริษัท':b['master_key'],'ยอด':b['subtotal'] or 0,
                '🔴':sum(1 for i in b['issues'] if i['severity']=='CRITICAL'),
                '🟠':sum(1 for i in b['issues'] if i['severity']=='ERROR'),
                'หมายเหตุ':' | '.join([f"[{i['code']}] {i['detail'][:50]}" for i in b['issues'] if i['severity']=='CRITICAL'][:2])})
        df = pd.DataFrame(rows)
        try:
    ...
    for b in all_bills:
        for i in b['issues']: cat_counts[i['category']][i['severity']] += 1
    if cat_counts:
        rows = []
    ...
    for idx, s in enumerate(summary, 1):
        bills_s = sorted(s['bills'], key=lambda b: (b['iv_date'] or datetime.max, b['sheet']))
        ...
        timeline = []
        for b in bills_s:
            sev = ('CRITICAL' if any(i['severity']=='CRITICAL' for i in b['issues'])
                   else 'ERROR' if any(i['severity']=='ERROR' for i in b['issues'])
                   else 'WARNING' if any(i['severity']=='WARNING' for i in b['issues'])
                   else 'INFO' if any(i['severity']=='INFO' for i in b['issues'])
                   else 'CLEAN')
            timeline.append({'วันที่':b['iv_date_str'] or '-','IV':b['iv_number'] or '-',
                            'ชีต':b['sheet'],'ยอด':b['subtotal'] or 0,
                            'Issues':len(b['issues']),'Severity':sev})
...
def _xlsx_sheet_dashboard(writer, all_bills):
    ...
    kpi = [
        ...
        {'Metric':'บิลปกติ','Value':sum(1 for b in all_bills if not b['issues'])},
        {'Metric':'บิลผิดปกติ','Value':sum(1 for b in all_bills if b['issues'])},
        {'Metric':'CRITICAL','Value':crit},
        {'Metric':'ERROR','Value':err},
        {'Metric':'WARNING','Value':warn},
        {'Metric':'INFO','Value':sum(1 for b in all_bills for i in b['issues'] if i['severity']=='INFO')},
        {'Metric':'ยอดก่อน VAT','Value':sum(b['subtotal'] or 0 for b in all_bills)},
        {'Metric':'VAT รวม','Value':sum(b['vat'] or 0 for b in all_bills)},
...
def _xlsx_sheet_highrisk(writer, all_bills_s):
    ...
    if hr:
        pd.DataFrame([{'งวดบัญชี': _get_p(b['iv_date']), 'วันที่':b['iv_date_str'],'ไฟล์':b['file'],'ชีต':b['sheet'],
            'IV': (b.get('iv_number_raw') or b.get('iv_number') or '-'),
            'บริษัท':b['master_key'],'ยอด':b['subtotal'] or 0,
            'Critical':sum(1 for i in b['issues'] if i['severity']=='CRITICAL'),
            'หมายเหตุ':' | '.join([f"[{i['code']}] {i['detail'][:80]}" for i in b['issues'] if i['severity']=='CRITICAL'])}

...
## ADR-193 — [FALSE-CLEAN] ITM001 มอง "ศูนย์" เป็น "ค่าหาย" → บรรทัด price=0 หลุดทั้ง ITM001 และ ITM018
วันที่: 2026-08-05
**อาการ:** บรรทัดรายการที่ `qty=5 price=0 amount=500` ไม่ถูกกฎใดฟ้องเลย — ยอด 500 บาทที่
  ขัดกับ `qty×price=0` ผ่านออกไปเป็น "ตรง" ทั้งที่ตรวจไม่ได้จริง (false-clean = คลาสที่ระบบนี้
  รับไม่ได้ที่สุด). พบด้วย oracle อิสระ `oracle_itm_matrix.py` (เมทริกซ์ "ผิดจริง → กฎไหนจับ").
**หลักฐาน:** `oracle_itm_matrix.py` 12 เคส — 11 เคสมีกฎจับ, เคส `price=0/amount=500` เงียบทั้งหมด
  (ก่อนแก้: `price=0 ทั้งที่ amount=500 → ผิด → (เงียบ) ❌ FALSE-CLEAN`)
**Root cause:** `rules_engine_rules_a.r_itm001` คัดบรรทัดด้วย **ความจริงเชิงตรรกะ**
  `if not (it['qty'] and it['price'] and it['amount']): continue`
  ใน Python `0`/`0.0` เป็น falsy เท่ากับ `None` → ค่าที่ "อ่านได้จริงจากเซลล์ว่าเป็นศูนย์" ถูกจัดชั้น
  เดียวกับ "ค่าหาย" แล้วข้ามเงียบ. เจตนาเดิมของบรรทัดนี้คือ "ข้ามเมื่อข้อมูลไม่ครบ" ซึ่งตรงกับ `is None`
  ไม่ใช่ความจริงเชิงตรรกะ.
  ตัวสำรองไม่ช่วย: `r_itm018` (rules_c) ครอบ `qty=0` และ `amount=0` ไว้ก็จริง แต่ **ทุกสาขาที่เทียบ
  `qty×price` ต้องการ `price_f > 0`** → `price=0` หลุดทั้งคู่. และ `r_itm008` จงใจไม่ตรวจตัวเลขเอง
  (คอมเมนต์ในโค้ด: "ปล่อย ITM001 จับเรื่องตัวเลขแทน") → ยิ่งพึ่ง ITM001 ตัวเดียว.
**ผลกระทบ:** corpus ที่ตรวจได้ (real_cases 80 บรรทัด / fixtures 3 บรรทัด) มี **0 บรรทัด** ที่มีศูนย์
  (`oracle_zero_lines.py`) → ไม่กระทบผลตรวจปัจจุบัน **แต่เป็นช่องโหว่แฝง**: เซลล์ราคา/ยอดที่ OCR
  อ่านเป็น 0 (ช่องว่าง/อ่านไม่ออก) จะกลายเป็น "ตรวจแล้วผ่าน" ทันทีที่เจอ. คลาสเดียวกับ ADR-189
  (เดาแล้วข้ามเงียบ) ต่างกันที่ครั้งนี้เกิดจาก type-truthiness ไม่ใช่ heuristic.
**การตัดสินใจ:** คัดด้วย `is None` (= ค่าหายจริง) แทนความจริงเชิงตรรกะ — แก้ 1 บรรทัดใน `r_itm001`
  ทางเลือกที่ไม่เลือก: (ก) ไปเพิ่มสาขา `price=0` ใน `r_itm018` — รักษาอาการ ไม่ใช่ต้นเหตุ และทิ้ง
  ITM001 ให้ตาบอดต่อไป ; (ข) ให้ parser แปลง 0 → None — ผิดกฎเหล็กข้อ 1 (ห้ามแต่งค่า) และทำลาย
  หลักฐานว่าเอกสารพิมพ์ 0 มาจริง.
**ผล:** `0×0=0` ยังลงตัว → บรรทัดศูนย์ล้วนยังเงียบ (ไม่เกิด false positive) ·
  golden fixture `d05e5de3…` **ไม่ขยับ** · เกตทั้งชุดเขียว
**เทส:** `test_adr193_itm001_zero_line.py` — 14 เช็ก: เคสบวก 6 (price/qty/amount=0 ที่ผิดจริง +
  ยืนยันว่ามีกฎจับอย่างน้อย 1 ตัว + ADR-189 ไม่ถอย) · เคสลบ 8 (ศูนย์ล้วน, ส่วนลดพิสูจน์ได้,
  `None` ต้องยังข้าม) — แดงก่อนแก้ 4 ข้อ เขียวหลังแก้
ขอบเขต: `rules_engine_rules_a.r_itm001` (1 บรรทัด + คอมเมนต์) · `test_adr193_itm001_zero_line.py` ·
  `run_ci.sh [4b6]` · oracle 4 ตัว (`oracle_suite` / `oracle_itm_matrix` / `oracle_zero_lines` /
  `oracle_failopen`) + allowlist ใน `test_reachability.py`

## ADR-194 — [FALSE-RED] เกต lint แดง 73 ข้อเพราะ "เครื่องมือคนละเวอร์ชัน" ไม่ใช่เพราะโค้ด
วันที่: 2026-08-05
**อาการ:** `run_ci.sh [11] ruff` แดง 73 ข้อ (BLE001/DTZ001/I001/UP006/UP009/UP035/SIM115/RUF022/
  PIE810) ทั้งที่โค้ดใน LINT_SCOPE ไม่ได้แก้เลย — ตรงกับ "งานค้างข้อ 1" ใน `CLAUDE_CODE_BUGHUNT.md`
**หลักฐาน:** ruff `0.16.1` เปิดกฎ **413 ตัว** (`ruff check --show-settings`) · pin ใน
  `requirements-dev.txt` คือ `ruff==0.15.19` ซึ่งเปิดเฉพาะ E,F.
  ยืนยันตรง ๆ: `pyproject.toml` เดิม + ruff 0.15.19 → **All checks passed** ·
  `pyproject.toml` เดิม + ruff 0.16.1 → 73 errors (ไม่แตะโค้ดเลยทั้งสองรอบ)
**Root cause (สองชั้น):**
  1. `run_ci.sh` เช็คแค่ว่า "มีเครื่องมือไหม" (`$PY -m ruff --version`) ไม่เคยเช็คว่า "ตรง pin ไหม" →
     ติดตั้งด้วย `pip install ruff` (ไม่ระบุเวอร์ชัน — ซึ่งเป็นวิธีที่คู่มือ bughunt §2 บอกให้ทำ)
     ได้ตัวใหม่กว่า pin แล้วเกตเปลี่ยนความหมายเงียบ. **ตระกูลเดียวกับ ADR-188** (mypy ไม่มี → SKIP
     เขียวหลอก) เพียงกลับด้าน: คราวนี้เป็น "แดงหลอก".
...
**ผลกระทบ:** ไม่กระทบผลตรวจ/golden เลย (คนละชั้น) แต่กินเวลาไล่บั๊ก และเสี่ยงกว่านั้นคือ
  ทำให้คนเคยชินกับ "เกตแดงเป็นปกติ" ซึ่งเป็นทางลื่นไปสู่การมองข้ามเกตที่แดงจริง
**การตัดสินใจ:**
  1. ประกาศสัญญา lint ให้ชัดใน `pyproject.toml`: `select = ["E4","E7","E9","F"]`
     (= สิ่งที่คอมเมนต์เดิมระบุอยู่แล้ว) → ชุดกฎคงที่ข้ามเวอร์ชัน. พิสูจน์แล้วว่า **ผลเท่าเดิมเป๊ะ**
     บน pin (All checks passed ทั้งก่อน/หลัง) และยัง passed บน 0.16.1 ด้วย
  2. เพิ่ม `qa_toolchain_gate.py` — เทียบเวอร์ชันเครื่องมือกับ pin: ไม่ได้ติดตั้ง = ข้าม (คงพฤติกรรม
     graceful เดิม) · ติดตั้งแล้วผิดเวอร์ชัน = **ตก พร้อมคำสั่งแก้**. เสียบเป็น `[9pre]` ก่อนเกต 10-13
  **ไม่ได้ทำ:** ไล่ "แก้" 73 ข้อนั้น — เพราะเป็นกฎสไตล์ที่โปรเจกต์ไม่เคยรับเป็นมาตรฐาน และบางข้อ
  ขัดข้อตกลงของโปรเจกต์โดยตรง (UP009 สั่งลบ `# -*- coding: utf-8 -*-` ที่ทุกไฟล์ใช้กับข้อความไทย ;
  DTZ001 สั่งใส่ tzinfo ทั้งที่ระบบ offline และตรึงวันที่ด้วย `PUOPUY_AUDIT_DATE`).
  การรับกฎเพิ่มต้องเป็นการตัดสินใจที่ตั้งใจ — ใส่ใน `select` ไม่ใช่ถูกเปิดให้เองตอนอัปเครื่องมือ
**ผล:** `bash run_ci.sh` → **✅ CI ผ่านทั้งหมด** (จากเดิมตก 1 ขั้น) · golden ไม่ขยับ ·
  ทดสอบเกตใหม่จริง: ตั้งใจลง ruff 0.16.1 → `[9pre]` ตก พร้อมบอก `pip install ruff==0.15.19`
ขอบเขต: `pyproject.toml` (`[tool.ruff.lint] select`) · `qa_toolchain_gate.py` (ใหม่) ·
  `run_ci.sh [9pre]` · allowlist ใน `test_reachability.py`

## ADR-195 — [เปิดค้าง · ต้องให้เจ้าของระบบตัดสิน] ยอดที่ระบบ "เติมเอง" ไม่มีกฎใดแจ้ง → รีพอร์ตขึ้น "ตรง"
วันที่: 2026-08-05 · สถานะ: **ยังไม่แก้ — รอเจ้าของระบบอนุมัติ** (ห้ามแก้เองตามกฎเหล็กข้อ 7)
**อาการ:** ใบกำกับที่ **ไม่มียอดท้ายบิลบนเอกสารเลย** ถูก parser เติมยอดให้เอง
  (`subtotal` = ผลรวมรายการ, `vat`/`total` = คำนวณจาก 7%) แล้ว **ไม่มีกฎใดฟ้องเลยแม้แต่ตัวเดียว** →
  ยอดที่ระบบแต่งขึ้นถูกนับรวมในรีพอร์ตและติดป้ายว่า "ตรง"
**หลักฐาน (reproduce ได้):**
  `python3 oracle_edge_corpus.py && python3 oracle_provenance.py _edge_corpus`
  · ชีต 2 (ไม่มียอดท้ายบิล): `amount_source = {'subtotal':'item_sum','vat':'derived','total':'derived'}`
    `amount_confidence=low` · `parse_confidence=MID` · **รหัสที่ฟ้อง = (เงียบ)**
  · รีพอร์ตจริง (`main.py` → `company_summary.txt`) พิมพ์ว่า
    `ยอด : 33,499 บาท ตรง` และ `บิล : 7 บิล ตรง`
    ทั้งที่ **4,000.00 บาท จาก 33,499.00 (11.9%)** ไม่เคยถูกอ่านจากเอกสารใด
    (33,499 = 4000×4 + 4000(ปลอม) + 9999 + 3500 — ยืนยันว่ายอดปลอมอยู่ในผลรวมที่ติดป้าย "ตรง")
**Root cause:** provenance ทำงานถูกต้อง (parser บันทึก `amount_source` ครบ — กลไกจาก ADR-182 ไม่พัง)
  แต่ **ผู้บริโภคของข้อมูลนั้นถูกปิด**: `r_vat010` ("VAT ไม่ได้ตรวจจริง (ยอดถูกคำนวณเอง)") ตั้ง
  `enabled=False` (`rules_engine.py:215` — คอมเมนต์: *"v9.1: ปิด/ลบการทำงานตามคำขอ"*).
  ผลคือ VAT001/002/003 เอา "ค่าที่ระบบเพิ่งคำนวณ" ไปเทียบกับตัวมันเอง → ตรงเสมอ = false-clean
  (คลาสเดียวกับ ADR-182 เป๊ะ เพียงมาทางช่องที่ยังไม่ถูกปิด)
**ทำไมยังไม่แก้:**
  1. กฎถูกปิด **ตามคำขอของเจ้าของระบบ** — การเปิดกลับเองขัดกฎเหล็กข้อ 7 (ห้ามตัดสินใจแทนเจ้าของ)
  2. `r_vat010` เวอร์ชันปัจจุบัน **กว้างเกินไป**: ฟ้องเมื่อ *ตัวใดตัวหนึ่ง* ใน subtotal/vat/total เป็น
     derived — แต่ `total:'derived'` (= sub+vat) เป็นเรื่องปกติของใบทั่วไป. วัดจริงบน corpus ที่มี:
     real_cases 5/15 ใบ และ edge 6/7 ใบ เป็น `total:'derived'` → เปิดตรง ๆ = flood false-positive
     (น่าจะเป็นเหตุผลเดิมที่ถูกปิด). สัญญาณที่ "แคบและถูกต้อง" คือ **`subtotal` หรือ `vat`
     ไม่ได้อ่านจากเอกสาร** (`!= 'ocr'`) — นั่นคือตอนที่ VAT001/002/003 เทียบตัวเองจริง ๆ
  3. เปลี่ยนชุดกฎ = ผลตรวจขยับบน corpus จริง 146 ไฟล์ ซึ่ง **ไม่มีในแพ็กนี้** จึงวัดผลกระทบไม่ได้
     → §5 ข้อ 9 บังคับให้หยุดแล้วรายงานก่อน
**ผลกระทบที่วัดได้:** corpus ที่ตรวจได้ในแพ็กนี้ (real_cases 15 ใบ / fixtures 3 ใบ) มี
  **0 ใบ** ที่ `subtotal`/`vat` ไม่ได้อ่านจากเอกสาร → **ช่องโหว่นี้ไม่กระทบผลตรวจปัจจุบัน**
  แต่จะกระทบทันทีที่เจอใบที่ OCR อ่านแถบยอดท้ายบิลไม่ติด (ซึ่ง ADR-182 เคยพบ 384 ใบ)
**ข้อเสนอให้เจ้าของระบบเลือก (ยังไม่ดำเนินการ):**
  · (ก) เปิด `VAT010` โดยแก้เงื่อนไขให้แคบลงเหลือ `subtotal`/`vat` เท่านั้น (ตัด `total:'derived'` ออก)
    — คาดว่า golden-neutral บน corpus ที่ใบส่วนใหญ่อ่าน sub+vat ได้ แต่ **ต้องวัดบน 146 ไฟล์จริงก่อน**
  · (ข) ปล่อยเป็น INFO/REVIEW แทน WARNING เพื่อไม่ดันใบเข้าเลน "ต้องแก้"
  · (ค) ให้รีพอร์ตแสดง "ยอดที่ระบบเติมเอง X บาท" ต่อท้ายบรรทัด `ยอด :` แทนการเพิ่มกฎ
    (แก้ความซื่อสัตย์ของรีพอร์ตโดยไม่แตะผลตรวจ/golden)
  · (ง) ยืนยันคงสภาพเดิม — ถ้าเจ้าของประเมินแล้วว่ารับความเสี่ยงนี้ได้
  **วิธีวัดก่อนตัดสินใจ:** `python3 oracle_provenance.py /mnt/project`
  → บอกจำนวนใบ + จำนวนเงินที่ระบบเติมเองบน corpus จริง ก่อนเปลี่ยนอะไร
ขอบเขต (เฉพาะเครื่องมือวัด — **ไม่แตะ production/กฎ/golden**): `oracle_provenance.py` ·
  `oracle_edge_corpus.py` · `oracle_rule_liveness.py` + allowlist ใน `test_reachability.py`

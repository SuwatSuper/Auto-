    ...
    try:
        ...
        with open(path, 'w', encoding='utf-8') as f:
            for iss in state._SYSTEM_ISSUES:
                # [ADR-173/BUGHUNT] try ต่อ record — เดิม record เดียวที่ serialize ไม่ได้ (surrogate จาก
                #   ชื่อไฟล์ decode ไม่ออก / ค่า non-JSON) ทำให้ sidecar ทั้งไฟล์หาย/ค้างครึ่งเดียว
                #   (เปิด 'w' truncate ไปแล้ว). ตอนนี้ข้ามเฉพาะ record เสีย ที่เหลือครบ.
                try:
                    rec = dict(iss); rec['logged_at'] = stamp
                    f.write(json.dumps(rec, ensure_ascii=False) + '\n')
                except Exception:
                    try:
                        f.write(json.dumps({'logged_at': stamp, 'code': 'SYS999',
                                            'name': 'Unserializable Record',
                                            'detail': repr(iss)[:300].encode('utf-8', 'replace').decode('utf-8')},
                                           ensure_ascii=False) + '\n')
                    except Exception:
                        pass
        return path

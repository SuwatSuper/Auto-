...
# ---------------------------------------------------------------------------
# [ADR-102] ลบบริษัทตัวอย่างทั้งหมดออกจาก master ตามคำสั่งเจ้าของ (2026-06-26):
#   บริษัทที่ตรวจแต่ละเดือนไม่ซ้ำ/ไม่เหมือนกัน → ระบบไม่ฝังบริษัทใดไว้เลย ผู้ใช้กรอกเองรายเดือน
#   ผ่าน  เพิ่ม_master.py  (วาง ภ.พ.20). golden baseline = พฤติกรรม "master ว่าง {}" = สถานะ default
#   จริงที่ ship (ผู้ใช้รันได้ทันทีโดยไม่มีบริษัท → ช่องตัวตนขึ้น "ไม่มี master ตรวจไม่ได้").
#   master-matching code path ยังถูกคุมโดย unit tests (test_match_guard / test_report_consistency /
#   test_report_summary_fixes / test_rules_coverage ฯลฯ ที่นิยาม fixture ทดสอบของตัวเอง — ไม่พึ่ง MASTER นี้).
MASTER = {}

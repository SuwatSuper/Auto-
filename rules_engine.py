...
from rules_engine_rules_c import (   # [F3 de-star] explicit — ครอบ __all__ ∪ internal ∪ rules_engine.X attr
    r_addr004, r_addr005, r_addr006, r_addr007, r_addr010, r_addr011, r_br003, r_cmp005,
    r_cmp007,
...
RULES = {
    ...
    'ADDR010':{'name':'จังหวัดไม่ใช่ 1 ใน 77 (สะกดผิด/ปลอม)','severity':'WARNING','category':'ที่อยู่','check':r_addr010,'enabled':True},
    # [ADR-190] สระอำเขียนแยกส่วน (นิคหิต+สระอา) ในช่องตัวตน — normalize เงียบมาตลอด ผู้ใช้ไม่เคยรู้
    'CMP007':{'name':'ตัวอักษรในชื่อบริษัทเขียนผิดรูป (สระอำแยกส่วน)','severity':'ERROR','category':'บริษัท','check':r_cmp007,'enabled':True},
    'ADDR011':{'name':'ตัวอักษรในที่อยู่เขียนผิดรูป (สระอำแยกส่วน)','severity':'WARNING','category':'ที่อยู่','check':r_addr011,'enabled':True},  # [ADR-122] จังหวัดในที่อยู่ต้องเป็นจังหวัดจริง (ไม่พึ่ง master). conservative
...
_XBILL_IDX_CACHE = {'fp': None, 'tax': {}, 'file': {}, 'iv': {}}   # [ADR-192] +iv index


def _xbill_indexes(all_bills):
    """คืน (tax_index, file_index, iv_index) ของ batch — สร้างครั้งเดียวแล้ว cache (เรียกซ้ำต่อบิล = O(1)).
    [ADR-192/PERF] เพิ่ม iv_index: DOC003 ตรวจ "ใบซ้ำข้ามไฟล์" (ADR-191) ต้องหาบิลที่เลข IV ตรงกัน —
      ถ้าเดิน all_bills ทุกครั้งจะเป็น O(n²) (n=5000 → 25 ล้านรอบ) ทำให้ perf canary เข้าใกล้เพดาน
      และจะพังจริงเมื่อ corpus ของผู้ใช้โตขึ้นในอนาคต. ดัชนีทำให้เหลือ O(1) ต่อบิล."""
    if not all_bills:
        return {}, {}, {}
    fp = (id(all_bills), len(all_bills), id(all_bills[0]), id(all_bills[-1]))
    if _XBILL_IDX_CACHE['fp'] != fp:
        tax_idx, file_idx, iv_idx = {}, {}, {}
        for ob in all_bills:
            t = clean_tax_id(ob.get('tax_id', ''))
            if t:
                tax_idx.setdefault(t, []).append(ob)
            file_idx.setdefault(ob.get('file'), []).append(ob)
            _iv = ob.get('iv_number')
            if _iv:
                iv_idx.setdefault(_iv, []).append(ob)
        _XBILL_IDX_CACHE.update(fp=fp, tax=tax_idx, file=file_idx, iv=iv_idx)
    return _XBILL_IDX_CACHE['tax'], _XBILL_IDX_CACHE['file'], _XBILL_IDX_CACHE.get('iv', {})
...
def run_rules(bill, master_companies, file_info, unit_index=None, all_bills_ref=None):
    ...
    # [ADR-172/BUGHUNT] type-gate iv_date — ตระกูล GAP-A ที่ตกหล่น: บิลภายนอกที่ iv_date เป็น str/serial
    #   (truthy แต่ไม่ใช่ date) ทำกฎวันที่ครัชเป็น SYS-* แล้วข้ามเงียบรายบิล (fuzz พบจริง: SYS-DOC001
    #   .day / SYS-DT001 .month / SYS-DT002 str>date). NaT (year=nan, self!=self) ตัดด้วย.
    #   parser จริงให้ datetime.date/None เสมอ → no-op บน corpus → golden-NEUTRAL.
    _ivd = bill['iv_date']
    if _ivd is not None and (not hasattr(_ivd, 'year') or _ivd != _ivd):
        bill['iv_date'] = None
    ...
    _xb_tax, _xb_file, _xb_iv = _xbill_indexes(all_bills_ref or [])
    ctx = {
        ...
        'xbill_iv_index': _xb_iv,      # [ADR-192] เลข IV → บิลที่ใช้เลขเดียวกัน (DOC003 ข้ามไฟล์)
    }

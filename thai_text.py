...
from config import (  # [F3] explicit (เดิม import *)
    CFG, CONSTRUCTION_DICT, EXPLAIN_CFG, FORMAL_CFG, OCR_CFG, PRODUCT_CATEGORIES,
    PYTHAINLP_WHITELIST)
...
def gen_explanation(subject, evidence='', action='', tier='', confidence_tier=''):
    parts = []
    state_intro = {
    ...
    intro = state_intro.get(subject, subject)
    if tier:
        intro += f' ({tier})'
    if confidence_tier:

...
def main():
    ...
    from golden_snapshot import MASTER as STUB, write_master_file
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            # ── เคส 3: stub รุ่นเก่า (ไม่มี marker — key+tax ตรง stub เป๊ะ) ──
            json.dump(STUB, open(mf, "w", encoding="utf-8"), ensure_ascii=False)
            importlib.reload(M)
            _patch_cfg(mf)
            m3 = M.load_master()
            _check("stub เก่าไม่มี marker (โครงตรงเป๊ะ) → False (legacy fallback)",
                   F._is_real_master(m3) is False)

...
def main():
    ...
    # [ADR-173/BUGHUNT] display 2 ตัวนี้เป็น advisory (console-only) แต่เดิม "ไม่ถูกห่อ" — บิลเพี้ยน
    #   (iv_date/เงินเป็น str, issues=None) ครัชตรงนี้ = ทั้ง run ตายหลังตรวจเสร็จ ก่อนได้รายงานใด ๆ
    #   (build_clean_report ที่รอดตั้งแต่ ADR-168 อยู่ถัดลงไป ยังไม่ทันถูกเรียก). หลัก ADR-166:
    #   advisory ห้ามฆ่างานหลัก.
    try:
        display_executive_dashboard(all_bills, summary, iv_issues, typos, filename_issues, len(file_list))
    except Exception as _e:
        print(f'⚠️ ข้ามแดชบอร์ดสรุป (advisory): {type(_e).__name__}: {_e}')

    # === แสดงบิลที่ระบบอ่านไม่ชัด ต้องเช็คด้วยมือ ===
    try:
        display_low_confidence_bills(all_bills)
    except Exception as _e:
        print(f'⚠️ ข้ามสรุปบิลความเชื่อมั่นต่ำ (advisory): {type(_e).__name__}: {_e}')
    ...
    if LEAN_REPORT:
        ...
        _report_ok = bool(build_clean_report(all_bills, summary, iv_issues, typos, filename_issues, out))
        if _report_ok:
            try:
            ...
            print('❌ สร้างรายงานคลีน (Excel) ไม่สำเร็จ — ดูสาเหตุจาก traceback ด้านบน')
            print('   → จะยังออก "หมายเหตุ (agent)" และ "สรุปบริษัท" ให้ตามปกติ (ไม่ผูกกับ Excel)')
        # [ADR-166] advisory outputs ต้อง "ไม่ตายตาม xlsx" — เดิมอยู่ใน branch สำเร็จเท่านั้น →
        #   วันที่ Excel พัง (ไฟล์ถูก lock/สไตล์ครัชกับข้อมูลใหม่) หมายเหตุหน่วยสินค้า+สรุปบริษัท
        #   หายไปด้วยทั้งชุด = ลูกค้าเห็นเป็น "ระบบไม่แจ้งเตือนหน่วยแล้ว" ทั้งที่ logic ปกติ.

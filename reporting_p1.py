...
from reporting_p0 import *  # noqa: F401,F403
...
def build_dashboard_figs(sheets, df_clustered):
    ...
    if '_cluster_id' in df_clustered.columns:
        d = df_clustered[df_clustered.get('_price_outlier', False) | df_clustered.get('_unit_rare', False)]
        if len(d) > 0:
            try:
...
<title>Anomaly Dashboard — Audit v5.8</title>
<style>
...
<div class="header">
  ...
  <p>Pattern Clustering + Plotly Interactive Charts • Audit System v5.8</p>
...
def _write_table(ws, rows, start_row=1, money_cols=(), int_cols=(), title=None,
    ...
    r = start_row
    if title:
        ws.cell(row=r, column=1, value=title).font = Font(name=CLEAN['FONT'], bold=True, size=13, color=CLEAN['INK'])
    ...
    if band_title:
        ...
        tc = ws.cell(row=r, column=1, value=band_title)
        ...
        sc = ws.cell(row=r+1, column=1, value=band_sub or '')
...
def _clean_sheet_duplicates(wb, all_bills):
    ...
    if dups:
        ...
        for (iv, amt, comp), bs in dups:
            d_rows.append({'IV':iv,'บริษัท':comp,'ยอดสุทธิ':amt,'พบซ้ำ':len(bs),

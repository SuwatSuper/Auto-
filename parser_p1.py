...
#   reexport bind object เดิม → golden ไม่ขยับ ; config = dependency จริงของ p1 (ไม่ใช่ chain) → คง explicit.
...
from puopuy_units import _money_q   # [F-1] ปัดเงิน HALF_UP แหล่งเดียว (แทน round() ใน _reconcile_amounts)
...
_IV_LONGDIGIT_RE = re.compile(r'\d{20,}')   # [ADR-174] บล็อกเลขติดกัน ≥20 หลัก = เชื้อ ReDoS (เกินเพดาน IV 13 หลักแน่นอน)
...
def _pick_best_iv(text, known_tax_id=None, return_score=False):
    ...
    candidates = []
    # [ADR-174/BUGHUNT] กัน ReDoS 2 ชั้น — เซลล์เดียวที่มีเลขติดกันยาวชนตัวอักษร (OCR junk) เดิมทำ
    #   backtracking ระเบิด (28 หลัก ~10s, 40+ หลัก = ค้างข้ามคืน ไม่มี wrapper ใดดัก hang ได้):
    #   (1) (?:…)* → {0,10}: จำกัดจำนวนกลุ่ม (IV จริง ≤4 กลุ่ม) — สตริงปกติ match เดิมเป๊ะ (fuzz ยืนยัน)
    #   (2) บล็อกเลขติดกัน ≥20 หลัก → แทน ￼ ก่อนสแกน: บล็อกแบบนี้เกินเพดาน IV (13 หลัก) แน่นอน
    #       เดิมโดน discard/ค้างอยู่แล้ว — เซลล์จริงไม่มี → s เดิมเป๊ะ. divergence เฉพาะสตริง pathological
    #       (ดู ADR-174 — เดิมคือเคสที่ "รันไม่จบ" จึงไม่มีพฤติกรรมเดิมให้รักษา).
    if _IV_LONGDIGIT_RE.search(s):
        s = _IV_LONGDIGIT_RE.sub('￼', s)
    for m in re.finditer(r'\b([A-Z]{0,5}[-\s]?\d{2,}(?:[-\s]?\d+){0,10})\b', s, re.IGNORECASE):
        ...
        # v9.3.2 [FIX-IV-FLOATTAIL]: หางทศนิยมยอดเงิน (เลข+จุดนำหน้า เช่น 75191.04000000001→'04000000001') ≠ เลขเอกสาร; s[pos-2]เป็นเลข→กัน DOC.04085
        _is_dec_tail = (cand[:1].isdigit() and pos >= 2 and s[pos-1] == '.' and s[pos-2].isdigit())
        candidates.append((cand, pos, m.group(1), _is_money or _is_dec_tail))
    ...
    ANTI_PREFIX = ['TAX', 'BR', 'ITEM', 'CODE', 'SKU', 'PRODUCT', 'REF',
                   ...
                   'MOO', 'RD', 'ID', 'NO',   # [BUGFIX recheck #3] +label ยอดเงิน (เช่น "VAT 999999" เคยถูกอ่านเป็นเลขใบกำกับ):
                   'VAT', 'NET', 'SUM', 'AMT', 'TOTAL', 'BAHT', 'GROSS', 'SUBTOTAL']
...
def _reconcile_amounts(sub, vat, tot):
    ...
    if have == 2:
        if sub is None: sub = _money_q(tot - vat)        # [F-1] HALF_UP แทน round() banker's
        elif vat is None: vat = _money_q(tot - sub)       # [F-1] (เคส SHS_68_117 ช.7: 8742.125→8742.13)
        elif tot is None: tot = _money_q(sub + vat)       # [F-1] HALF_UP แทน round()
        return sub, vat, tot, 'high'
...
def _pb_try_address_line(addr_lines, s):
    ...
    if re.search(r'(เลขที่\s*\d+|ตำบล|แขวง|หมู่ที่\s*\d+)', s):
        ...
        if s not in addr_lines: addr_lines.append(s)
    # [ADR-125 FIX-ADDRLINE] บรรทัดที่อยู่ "บรรทัดบน" ที่ขึ้นต้นด้วยบ้านเลขที่เปล่า (ไม่มี label 'เลขที่')
    #   + มีคำพักอาศัย (ซอย/ตรอก/ถนน/หมู่บ้าน/หมู่ที่/อาคาร) แต่ anchor แข็ง (แขวง/เขต/จว.) หลุดไปบรรทัดล่าง
    #   เดิมตกทุก branch ด้านบน → ทิ้งทั้งบรรทัด → บ้านเลขที่หาย → ADDR001 ฟ้อง "ไม่พบเลขที่" หลอก (KNT/CETI).
    #   guard เข้ม: ต้อง "ขึ้นต้นด้วยเลข" (กันบรรทัดสินค้า เช่น 'สีทาถนน...'/'ขิงซอย' ที่ไม่ได้ขึ้นต้นด้วยเลข)
    #            + มีคำพักอาศัย + len<200. corpus blast=0 → golden-neutral (พิสูจน์ด้วย regression_full).
    elif (re.match(r'^\s*\d+(?:[/\-]\d+)*\b', s)
          and re.search(r'(ซอย|ตรอก|ถนน|หมู่บ้าน|หมู่ที่|อาคาร)', s)
          and len(s) < 200):
        if s not in addr_lines: addr_lines.append(s)
...
def _pb_scan_header(df, result, addr_lines, row_start, header_end, ncols):
    ...
    for r in range(row_start, header_end):
        for c in range(ncols):
            ...
            # [ADR-190] เก็บ "ค่าดิบ" ของบรรทัดที่อยู่ไว้ด้วย — `normalize_text` แปลง
            #   นิคหิต+สระอา → สระอำ เงียบ ๆ (จำเป็นต่อการ match) ทำให้ result['address']
            #   ที่ประกอบจาก addr_lines ไม่เหลือร่องรอยว่าเอกสารสะกดผิดอักขระ → ADDR011 ตรวจไม่ได้.
            #   เก็บเฉพาะบรรทัดที่ "ถูกรับเข้า addr_lines จริง" ในรอบนี้ (เทียบความยาวก่อน/หลัง).
            if addr_lines and addr_lines[-1] == s:
                _rawline = str(v).strip()
                if _rawline and _rawline not in result.setdefault('_addr_raw_lines', []):
                    result['_addr_raw_lines'].append(_rawline)
            ...
            if not result['iv_date']:
                # [ADR-055 DT-FIX] เซลล์ตัวเลขเปล่าช่วง Excel-serial ค.ศ. (30000-70000) แทบทั้งหมด = "ยอดเงิน"
                #   (เช่น line-total 43600 → 2019-05-15) ไม่ใช่วันที่ → กัน money-misread ที่ call-site นี้
                #   เท่านั้น (ไม่แตะ parse_date_any/characterization; พ.ศ.-serial>200000 ไม่กระทบ). scope=1 บิล.
...
def _pb_build_item(block_df, r, seq, name_col, qty_col, unit_col, price_col, amt_col):
    ...
    # [ADR-189/DISCOUNT] เก็บ "ส่วนลดที่พิมพ์บนบรรทัด" เพื่อให้ ITM001 *พิสูจน์* ยอดได้ แทนการ *เดา*.
    #   ปัญหาที่แก้: r_itm001 เดิม ถ้า qty×price ≠ amount แล้วยอดอยู่ในช่วง 50–99% ของ qty×price
    #   จะ "สันนิษฐานว่าเป็นส่วนลด" แล้วข้ามเงียบ โดยไม่เคยดูว่าเอกสารมีส่วนลดจริงไหม
    #   → ยอดที่ผิดจริง 1–50% ถูกกลืนหายทั้งแถบ (blind spot กว้างมากสำหรับระบบตรวจสอบ).
    #   เชิงประจักษ์: corpus นี้มี 18 บรรทัดที่ qty×price ≠ amount — ทั้ง 18 มีคอลัมน์ส่วนลดจริง
    #   (TKH_69_05 คอลัมน์ 14 = 0.25/0.20) และกระทบยอดได้เป๊ะ → ของเดิม "บังเอิญถูก" ไม่ใช่ "ตรวจแล้วถูก".
    #   วิธีหา: สแกนเซลล์ตัวเลขในแถวเดียวกันที่ทำให้ qty×price×(1−d) = amount พอดี (รับทั้งรูป 0.25 และ 25)
    #   ไม่เจอ → None → ITM001 ฟ้องตามปกติ (ไม่เดาแทนผู้ใช้ ไม่กลบ error).
    disc = None
    if (isinstance(qty, (int, float)) and isinstance(price, (int, float))
            and isinstance(amt, (int, float)) and qty * price > 0 and abs(qty * price - amt) > 0.01):
        _e = qty * price
        for _c in range(block_df.shape[1]):
            _v = _cell_to_num(block_df.iat[r, _c]) if not pd.isna(block_df.iat[r, _c]) else None
            if _v is None or _v <= 0:
                continue
            for _f in (_v, _v / 100.0):
                if 0 < _f < 1 and abs(_e * (1 - _f) - amt) <= 0.01:
                    disc = _f
                    break
            if disc is not None:
                break
    return {
        ...
        'price':price,
        'amount':amt,
        'discount':disc,       # [ADR-189] สัดส่วนส่วนลดที่ "พิสูจน์ได้จากเอกสาร" (None = ไม่มี/พิสูจน์ไม่ได้)
    }

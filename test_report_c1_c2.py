...
_check("C2 หน่วยขาด → ต้องขึ้นช่องรายการสินค้า ไม่ขึ้น 'ตรง' หลอก [ADR-186]",
       "รายการสินค้า : ตรง" not in blk_ms and "ไม่มีหน่วย" in blk_ms)
_check("C2 หน่วยขาด → ไม่มีบรรทัด 'ตรวจตาเพิ่ม' แล้ว (เลิกชั้นก้ำกึ่ง)",
       "ตรวจตาเพิ่ม" not in blk_ms and "รีเช็ครายการสินค้า" in blk_ms)

print("=" * 60)
if _fail == 0:
    print("RESULT: [PASS] C1 รายงานระบุไฟล์ + C2 หน่วยผิด/ขาด ขึ้นช่องรายการสินค้าตามฟอร์ม")
    sys.exit(0)

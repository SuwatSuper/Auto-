...
(สรุปงาน: 🔴 เปลี่ยน join master จาก fuzzy-name → tax_id · 🟡 เพิ่ม identity-golden coverage · 🟡 เสริม test parse ภ.พ.20 — ทั้งหมด golden-neutral, main golden `814c7cf0` ห้ามขยับ)
...
2. ติดตั้ง dep (มีเน็ต, เวอร์ชันตรึง):
   ...
   ```
3. **ด่าน sanity ก่อนแตะโค้ด** — ต้องได้ golden `814c7cf0`:
   ```bash
...
ทุกอย่างที่จำเป็นต่อ golden `814c7cf0` (โค้ด ADR-145 + `corpus/` 152 + `baseline.json` + `.vscode/` + `constraints.txt`) อยู่ครบ.

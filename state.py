...
# [ADR-PMERGE] dedup key จริงของแต่ละ issue เรียงตรงกับ _SYSTEM_ISSUES 1:1 (grow คู่กันใน logger)
#   มีไว้ให้ parallel merge รวม system issue ข้าม worker ด้วย "key จริง" (รวม exc-type ที่ไม่ได้เก็บใน dict)
#   → เลิก reconstruct key จาก dict (เดิมผิดสำหรับ issue ที่มาจาก exception). sidecar ล้วน:
#   ไม่กระทบ serial / รายงาน / sidecar JSONL / golden — แค่ trail คู่ขนานสำหรับ merge.
#   reset ด้วย .clear() ใน system_issues_reset() (คง identity เหมือน _SYSTEM_ISSUES)
_SYSTEM_ISSUE_KEYS = []      # [(code, file, sheet, exc_type_or_name), ...] 1:1 กับ _SYSTEM_ISSUES

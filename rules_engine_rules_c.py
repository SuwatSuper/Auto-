...
def r_vat006(b,m,c):
    ...
    items_sum = sum(i['amount'] for i in b['items']
                    if isinstance(i.get('amount'), (int, float)))
    if items_sum <= 0: return []
    expected_inclusive = round(tot / 1.07, 2)
    if (abs(items_sum - expected_inclusive) < 1 and isinstance(sub, (int, float))
        and abs(items_sum - sub) > 1):
...
def r_vat007(b,m,c):
    ...
    items_sum = sum(i['amount'] for i in b['items']
                    if isinstance(i.get('amount'), (int, float)))
    if items_sum <= sub: return []
    discount = items_sum - sub
    exp_vat_post = round(sub * 0.07, 2)
    exp_vat_pre = round(items_sum * 0.07, 2)
    if abs(vat - exp_vat_pre) < 0.5 and abs(vat - exp_vat_post) > 0.5:
    ...
    try:
        ...
        for it in b['items']:
            ...
            if qty_f > 0 and price_f is not None and amount_f is not None and price_f > 0:
                ...
                if abs(expected - amount_f) > 1:
                    pct = abs(expected - amount_f) / expected * 100 if expected != 0 else 0
                    # skip ถ้าเหมือน discount 1-50%
                    if amount_f < expected and 1 <= pct <= 50:
                        continue

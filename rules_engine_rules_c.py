...
import os   # [ADR-191] basename ไฟล์ในข้อความ DOC003 ข้ามไฟล์
...
# [ADR-118] เดิม 'สาขา\S*' (unanchored) กิน 'สาขา' กลางชื่อจริง (เช่น 'บริษัท สาขาวิชาการ จำกัด' → ยุบเหลือ
#   'บริษัทจำกัด') → คนละนิติบุคคลยุบชนกัน = TAX009 false-positive (ฟ้อง 'ชื่อเดียวเลขต่าง' ผิด) + TAX008
#   false-negative (พลาดสวมเลขจริง). แก้: ตัด marker เฉพาะ (ก) ในวงเล็บ (รวมสาขาตั้งชื่อ) (ข) HQ keyword
#   (ค) 'สาขา[ที่] <เลขสาขา>' — ไม่กิน 'สาขา<คำ>' กลางชื่อ. พ่วงแก้ residue เลขสาขา (เช่น 'สาขา 00001'
#   เดิมเหลือ '00001' ในชื่อ). พิสูจน์: normalize 66 ชื่อใน corpus ไม่ขยับ (golden-neutral 31013a31).
_TAX008_BRANCH_RE = re.compile(
    r'\(\s*(?:สำนักงานใหญ่|สนญ\.?|สาขา[^)]*)\s*\)'   # marker ในวงเล็บ (รวม (สาขาสีลม) ตั้งชื่อ)
    r'|\s*สำนักงานใหญ่\s*'                            # HQ เต็มคำ (นอกวงเล็บ)
    r'|\s*สนญ\.?(?=\s|$)'                             # ตัวย่อ HQ (ขอบคำ)
...
def r_doc003(b, m, c):
    ...
    try:
        ...
        # [ADR-191/ข้ามไฟล์] ใบเดียวกันโผล่คนละไฟล์ = "นับยอดซ้ำ" — ต้องฟ้องในรีพอร์ตหลัก
        #   เดิม v8.6 จำกัด DOC003 ไว้ "ไฟล์เดียวกัน" เพราะกลัว flood จากไฟล์สำเนา แล้วอ้างว่า
        #   "ชีตบิลซ้ำใน Excel จับอยู่แล้ว → ไม่เสียสัญญาณ". เชิงประจักษ์พบว่า **เสียสัญญาณจริง**:
        #   company_summary.txt (รีพอร์ตหลักที่บัญชีใช้) ขึ้น "เลขที่ : ตรง / เลขที่ iv : ตรง /
        #   ยอด : 32,238,324.60 บาท ตรง / บิล : 100 บิล ตรง" สำหรับ บ.สยามมิตร สตีล รีไซเคิล 05/69
        #   ทั้งที่มีใบซ้ำ 18 ใบ (SEE_69_053↔SEE_69_055, HSH_69_05↔HSH_69_051) รวม 6,704,368.50 บาท
        #   ถูกนับสองครั้ง — ถ้าใช้ยอดนี้เครมภาษีซื้อ = ผิดกฎหมาย. Excel ชีตแยกที่ผู้ใช้อาจไม่เปิด
        #   ไม่ใช่ "การแจ้ง" ที่เพียงพอสำหรับความเสี่ยงระดับนี้.
        #   เงื่อนไขเข้มเพื่อกัน false positive: ผู้ขายเดียวกัน (tax_id) + เลข IV เดียวกัน +
        #   วันที่เดียวกัน + **ยอดก่อน VAT เท่ากันเป๊ะ** → เป็นใบเดียวกันแน่นอน ไม่ใช่เลขชนกันบังเอิญ
        #   (เลขชนข้ามผู้ขายมี 28 คู่ใน corpus นี้ ยอดต่างกัน → ไม่เข้าเงื่อนไข ไม่ถูกฟ้อง).
        _sub = b.get('subtotal')
        if _sub is None:
            return []
        xfile = []
        # [ADR-192/PERF] เดินเฉพาะบิลที่ "เลข IV ตรงกัน" ผ่านดัชนี (O(1)) แทน scan ทั้ง batch (O(n))
        #   → กฎนี้กลับมาเป็น O(n) ทั้งรอบแทน O(n²) ; fallback เป็น all_bills ถ้าไม่มีดัชนี
        for ob in c.get('xbill_iv_index', {}).get(this_iv, all_bills):
            if ob is b or ob.get('file') == b.get('file'):
                continue
            if ob.get('iv_number') != this_iv:
                continue
            if clean_tax_id(ob.get('tax_id', '')) != this_tax:
                continue
            if _dkey(ob) != this_date:
                continue
            _osub = ob.get('subtotal')
            if _osub is None or abs(float(_osub) - float(_sub)) > 0.01:
                continue
            xfile.append(os.path.basename(str(ob.get('file', '?'))))
        if xfile:
            return [f"ใบนี้มีอยู่ในไฟล์ {'/'.join(sorted(set(xfile))[:2])} ด้วย "
                    f"(ยอด {float(_sub):,.2f} ถูกนับซ้ำ) ต้องเลือกใช้ไฟล์เดียว"]
        return []
    except Exception:
        return []
...
# ── [ADR-190] ตัวอักษรไทยเขียนผิดรูปในช่องตัวตน (ชื่อบริษัท / ที่อยู่) ────────────────
#   บริบท: `normalize_text` แปลง นิคหิต(U+0E4D)+สระอา(U+0E32) → สระอำ(U+0E33) เงียบ ๆ
#   เพื่อให้ match ข้อความได้ (จำเป็น ห้ามถอด — ถ้าถอด ชื่อ 'สํานักงานใหญ่' จะจับคู่ไม่ติด).
#   ปัญหา: "เงียบ" = ผู้ใช้ไม่เคยรู้ว่าเอกสารต้นฉบับสะกดผิดอักขระ. เชิงประจักษ์ corpus 146 ไฟล์
#   พบ 57 เซลล์ใน 4 ไฟล์ — 'บริษัท โปร-พลัส พร็อพเพอร์ตี้ จำกัด (สํานักงานใหญ่)' ×16 ·
#   'อําเภอบางบัวทอง จังหวัดนนทบุรี 11110' ×16 · 'ตําบลท่าตูม อําเภอศรีมหาโพธิ' ×1 · ชื่อบริษัทอีก 1.
#   ผลกระทบจริง: จับคู่ ภ.พ.20 / ค้นหา / ส่งออกไประบบบัญชีอื่น จะ "ไม่ตรง" ทั้งที่ตาเห็นเหมือนกัน
#   → เป็น error ที่ต้องแจ้งตามฟอร์ม (ไม่ใช่ข้อสังเกต) สำหรับชื่อบริษัท ; ที่อยู่ = check.
_MALFORMED_SARA_AM = '\u0e4d\u0e32'


def _has_malformed_thai(s) -> bool:
    """True ถ้าข้อความมี สระอำ เขียนแบบแยกส่วน (นิคหิต + สระอา) ซึ่งเรนเดอร์เหมือน ำ แต่คนละอักขระ"""
    if not s:
        return False
    try:
        return _MALFORMED_SARA_AM in unicodedata.normalize('NFC', str(s))
    except (TypeError, ValueError):
        return False


def r_cmp007(b, m, c):
    """ชื่อบริษัทบนบิลมีอักขระผิดปกติ — ตรวจจากค่าดิบ (company_raw) ที่ยังไม่ถูก normalize.
    ใช้ has_hidden_chars ตัวเดียวกับ ITM004 → ครอบ NBSP/ZWS/BOM/สระอำแยกส่วน ครบชนิดเดียวกัน
    (ของเดิมช่องชื่อบริษัทไม่เคยถูกตรวจอักขระเลย — มีแต่ช่องรายการสินค้า)"""
    raw = b.get('company_raw') or ''
    hidden = has_hidden_chars(raw)
    if hidden:
        return [f"ชื่อบนบิล \"{str(raw)[:55]}\" มีอักขระผิดปกติ: {'; '.join(hidden)} "
                f"— ตาเห็นเหมือนกันแต่คนละอักขระ ทำให้จับคู่ทะเบียน/ค้นหาไม่ตรง"]
    return []


def r_addr011(b, m, c):
    """ที่อยู่บนบิลมีอักขระผิดปกติ (เช่น 'อําเภอ' ที่ควรเป็น 'อำเภอ') — ตรวจจาก address_raw ก่อน"""
    for _k in ('address_raw', 'address'):
        raw = b.get(_k) or ''
        hidden = has_hidden_chars(raw)
        if hidden:
            return [f"ที่อยู่ \"{str(raw)[:55]}\" มีอักขระผิดปกติ: {'; '.join(hidden)}"]
    return []

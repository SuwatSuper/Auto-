...
ทำไมต้องมี (P1):
  golden master baseline (hash `ec61907f…`) ถูกพิสูจน์บน "ชุดเวอร์ชันที่ล็อกไว้" (config._LOCKED).
...
ปรัชญาการจัดระดับ (ไม่ทุก dependency อันตรายเท่ากัน — พิสูจน์เชิงประจักษ์):
  ...
  • OPTIONAL_CRITICAL = pythainlp
        ...
        (เชิงประจักษ์: ขาด pythainlp ใน CI นี้ hash ยังตรง baseline → ไม่ทำให้ FAIL โดยไม่จำเป็น)
...
def format_report(rep: Report) -> str:
    ...
    if rep.failures:
        ...
        lines.append("  ⚠️ มีความต่างระดับไม่อันตราย (ไม่กระทบ golden hash) — รันต่อได้:")

...
WHITELIST = {
    ...
    # reporting_p0.py: report/display layer ล้วน (advisory/golden-neutral). ADR-173 เพิ่มเกราะบิลเพี้ยน
    #   (issues=None ×28 จุด · _fin0 เงินปลอดภัย · _get_p/_bill_date_key guard · str company) ปิดคลาส
    #   "advisory ครัชฆ่า run หลังตรวจเสร็จ" → เดิมชิดเพดาน (~590) เป็น 610. ซอยปลอดภัย.
    #   แผน: รอบ maintenance ถัดไปแยก display_* (console/HTML) → reporting_display.py.
    'reporting_p0.py': 'report/display layer (golden-neutral); ADR-173 เกราะบิลเพี้ยนทั้ง display+full; แผนแยก display_* → reporting_display.py รอบ maintenance ถัดไป',
}

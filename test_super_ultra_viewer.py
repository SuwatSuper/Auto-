...
# ── [ADR-186/NO-SOFT-LANE] เลิกชั้น "ตรวจตาเพิ่ม" — ทุกจุดขึ้นบรรทัดช่องตัวเองตามฟอร์ม ──
#   คำสั่งเจ้าของระบบ: "ไม่ต้องตรวจตาเพิ่ม — อะไรผิดก็ต้องบอกผิด ตามฟอร์ม".
#   เดิม ITM011 (ก้ำกึ่ง) ทำให้ช่องหลักขึ้น "ตรง" + ยกไปบรรทัดแยก = รีพอร์ตขัดกันเองในบล็อกเดียว.
#   เทสนี้ตรึงสัญญาใหม่: ช่องต้องระบุจุด (ห้ามขึ้น "ตรง") · ห้ามมีบรรทัด "ตรวจตาเพิ่ม" · footer ต้องรีเช็ค.
...
_check("ITM011: ช่องรายการสินค้าต้องระบุจุด ไม่ขึ้น 'ตรง' หลอก",
       "รายการสินค้า : ตรง" not in _blk_s and "เจียร" in _blk_s)
_check("ไม่มีบรรทัด 'ตรวจตาเพิ่ม' / 'ก้ำกึ่ง' เหลืออยู่ (เลิกชั้นก้ำกึ่งแล้ว)",
       "ตรวจตาเพิ่ม" not in _blk_s and "ก้ำกึ่ง" not in _blk_s)
_check("footer ต้องบอกให้รีเช็ค (ห้ามสรุปว่า 'ตรงครับ' ทั้งที่ยังมีจุดฟ้อง)",
       "รีเช็ครายการสินค้าครับ" in _blk_s and "ตรงครับ (มี" not in _blk_s)
# ITM010 (ชัด) ยังขึ้นรีพอร์ตหลักเหมือนเดิม
...
_check("clear (ITM010 ชัด): ยังขึ้นรีพอร์ตหลัก + footer รีเช็ค",
       "คำว่ามั้วน" in _blk_c and "รีเช็ครายการสินค้าครับ" in _blk_c
       and "ตรวจตาเพิ่ม" not in _blk_c)
# [ADR-185/NO-HIDE] ITM015/ITM016/ITM018 ต้องไม่ถูกซ่อนจาก .txt อีก (เดิม _HIDE_IN_SUMMARY ตัดทิ้ง)
_hid_bill = dict(clean_bill)
_hid_bill["file"] = "TSH_69_05.xls"
_hid_bill["issues"] = [{"code": "ITM015", "detail": '#1 "ทรายหยาบ" — ใช้หน่วย [\'คิว\', \'ตัน\'] ในชุดเดียวกัน',
                        "severity": "WARNING"}]
_blk_h = SUV.render_block(1, SUV.build([_hid_bill])[0])
_check("ITM015 ต้องขึ้นรีพอร์ตหลัก (เลิกซ่อน — ห้ามขึ้น 'ตรง' ทั้งที่มีจุดฟ้อง)",
       "รายการสินค้า : ตรง" not in _blk_h and "รีเช็ครายการสินค้าครับ" in _blk_h)
_check("_HIDE_IN_SUMMARY/_XLSX_ONLY ต้องว่าง (ไม่มีจุดที่มองไม่เห็นใน .txt)",
       not SUV._HIDE_IN_SUMMARY and not SUV._XLSX_ONLY)

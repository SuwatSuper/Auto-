...
# [งาน D] ItemViewer: ITM015 → 'หน่วยสินค้าผิด' ; ITM002 → 'ลำดับรายการสินค้าหาย'
...
_check("ITM002 → 'ลำดับรายการสินค้าหาย'", set(v["fix"]) == {"ลำดับรายการสินค้าหาย"})
...
blk = SUV.render_block(1, rows[0])
_check("บล็อกปัญหา: รายการสินค้า 'ระบุจุด' (ลำดับที่ + → + รีเช็คครับ) ไม่มี emoji",
       "ลำดับที่ 2" in blk and "→" in blk and "รีเช็คครับ" in blk
...
_check("บล็อก .txt ระบุ prefix ('C') ไม่ใช่ชื่อไฟล์เต็ม ('C.xls'), ไม่มี 📌",
       "ไฟล์ C " in blk and "C.xls" not in blk and "📌" not in blk)
...
_check("no-master: ไม่พิมพ์ 'ตรงครับ ✅' หลอก", "ตรงครับ ✅" not in blk_nm)

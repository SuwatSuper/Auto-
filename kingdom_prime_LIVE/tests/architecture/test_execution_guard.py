...
def test_only_execution_agent_imports_bitkub_rest() -> None:
    """bitkub_rest must only be referenced by execution_agent.py."""
    offenders: list[str] = []
    for path in _py_files():
        if path == _BITKUB_REST:
        ...
        text = path.read_text(encoding="utf-8", errors="ignore")
        if "bitkub_rest" in text:
            offenders.append(str(path.relative_to(SRC)))
    assert not offenders, f"bitkub_rest referenced outside execution_agent: {offenders}"

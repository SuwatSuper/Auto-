...
# [ADR-102] golden_snapshot.MASTER ว่างแล้ว → เทสนี้นิยาม master fixture ของตัวเอง (master-matching path)
MASTER = {
    "ฉี อัน คอนสตรัคชั่น": {
...
# [C1] ต้องระบุ "ไฟล์" ด้วย (เปิดไปแก้ถูกจุด — เดิม '(N บิล)' ลอย ๆ)
check('ไฟล์ TKH' in s9, f"ระบุชื่อไฟล์ (prefix) ({s9})")
# ผิดแค่ field เดียว + ระบุไฟล์
s1 = addr_summary([{'file': 'TSH', 'sheet': '1', 'date': 'x',
                    'detail': 'ที่อยู่ไม่ตรงทะเบียน: รหัสไปรษณีย์ไม่ตรง (บิล: 10540 / ทะเบียน: 10280)'}])
check(s1.startswith('ไฟล์ TSH รหัสไปรษณีย์ ไม่ตรง (1 บิล)'), f"ผิด field เดียว → ไฟล์+สั้นตรง ({s1})")

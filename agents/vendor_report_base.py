...
def _be_period(d) -> Optional[Tuple[int, int]]:
    ...
    [ADR-173/BUGHUNT] int() ครอบ — pd.NaT มี year/month แต่เป็น nan → เดิม format ครัช
    ล้ม vendor .txt ทั้งชุด (twin ของ _clean_period/ADR-170). date ปกติผลเท่าเดิมเป๊ะ."""
    if d is not None and hasattr(d, "year") and hasattr(d, "month"):
        try:
            yr = int(d.year); mo = int(d.month)
...
def _full_company(vbills: List[dict]) -> str:
    # [ADR-173/BUGHUNT] str() — company non-str (int/float จากบิลเพี้ยน) เดิม .strip() ครัชล้ม vendor report
    names = [str(b.get("company") or "").strip() for b in vbills if str(b.get("company") or "").strip()]
    if not names:
...
def _is_noise_issue(iss: dict) -> bool:
    ...
    if base == "ITM005":  # เดาหน่วย — soft เสมอ
        return True
    # [ADR-098] ITM015 = ใช้หน่วยปนกันในชุดสินค้าเดียวกัน (เช่น หิน 3/4 ขายทั้ง คิว/ตัน) —
    #   ไม่ใช่ error ของบิล (สินค้าจริงขายได้หลายหน่วย). super_ultra_viewer (company_summary)
    #   ซ่อน ITM015 อยู่แล้ว (_HIDE_IN_SUMMARY) → กรองที่นี่ด้วยให้รีพอร์ตสองฉบับสอดคล้องกัน.
    if base == "ITM015":
        return True

...
from code_labels import MASTER_DEPENDENT_FIELDS, uncheckable_reason, group_master_status  # [A1] honesty 'ตรง'=เทียบ master จริง
...
def _render_one(
    ...
    *,
    ...
    # [ADR-101] matched ใช้ group_master_status เดียวกับ company_summary → 2 รีพอร์ตสอดคล้องกันโดยโครงสร้าง.
    #   เดิม vendor พึ่ง master ที่ caller ส่งมาเท่านั้น — ถ้า caller ลืมส่ง (เช่น สคริปต์ทดสอบ) ช่องตัวตน
    #   ขึ้น "ตรวจไม่ได้" หลอก ทั้งที่ engine แมตช์บิลกับ master แล้ว (b['master_key']). master_key คือสัญญาณ
    #   matched ที่ถูกต้อง (engine ตั้งจาก match_company) ; _ventry ยังหาจาก _find_master_entry เพื่อเช็ค
    #   "ทะเบียนมีช่องนี้ไหม" เมื่อ master ถูกส่งมา.
    _gmatched, _gkey, _gentry = group_master_status(vbills, master_present=bool(master), masters=master)
    _ventry = _gentry if _gentry is not None else _find_master_entry(vbills, master)
    _vmatched = _gmatched

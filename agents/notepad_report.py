...
def _render_unit_consistency(bills) -> List[str]:
    ...
    # [ADR-104/P1] (2) ปนภาษาไทย+อังกฤษ — ราย "ไฟล์" (ใบกำกับ) ไม่ใช่ราย "บริษัท".
    #   เดิมรวมข้ามไฟล์ระดับบริษัท → FP: ผู้ขายต่างเจ้าเขียนหน่วยคนละภาษา (m vs เมตร) ทำให้บริษัท
    #   ถูกหาว่าปนภาษาทั้งที่แต่ละใบสคริปต์เดียวสม่ำเสมอ. ใหม่: ชี้ "ไฟล์" ที่หน่วยเดียวกัน (family
    #   เดียว) ถูกเขียน 2 สคริปต์ในใบเดียว = ของจริงที่ตรวจสอบได้ + บอกชื่อไฟล์ชัด.
    comp_mix = ux.detect_company_unit_language_mix(bills)   # ใช้ต่อสำหรับ (2b) หน่วยขาดราย "บริษัท"
    blank_any = [d for d in comp_mix if d["blank"]]
    file_mix = ux.detect_file_unit_language_mix(bills)
    if file_mix:
        out.append("   ⚠️ ไฟล์ที่ใช้หน่วยสินค้า (หน่วยเดียวกัน) ปนภาษาไทย+อังกฤษ — ควรใช้รูปเดียวกัน:")
        for d in file_mix:
            for fam in d["mixed_families"]:
                pair = " / ".join(list(fam["th"]) + list(fam["en"]))
                out.append(f"      • {d['file']} — หน่วย “{fam['family']}” เขียนปน: {pair}")
    else:
        out.append("   ✅ ไม่พบไฟล์ที่ใช้หน่วยสินค้าเดียวกันปนภาษาไทย+อังกฤษ")
    out.append("")

    # (2b) หน่วยขาด/ดึงไม่ครบ — ราย "บริษัท"
    if blank_any:
        out.append("   ⚠️ บริษัทที่มีหน่วยสินค้าขาด (ช่องหน่วยว่างในต้นฉบับ):")
        for d in blank_any:
            out.append(f"      • {d['company']} — {d['blank']} รายการไม่มีหน่วย")
    else:
        out.append("   ✅ ไม่พบหน่วยสินค้าขาด")
    out.append("")

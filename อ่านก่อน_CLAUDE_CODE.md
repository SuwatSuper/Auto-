...
3. Claude Code จะทำตามขั้น: ติดตั้ง dep (มีเน็ต) → ด่าน sanity (ต้องได้ golden `f05358aa`) → harden 3 รอบ → ผ่าน gate ครบ → re-vendor → `make_release` → ส่งระบบสมบูรณ์.
...
ทุกอย่างที่จำเป็นต่อการ reproduce golden `f05358aa` (โค้ด + `corpus/` 152 ไฟล์ + `baseline.json` + `constraints.txt`) อยู่ครบในชุดนี้.

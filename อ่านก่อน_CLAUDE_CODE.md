...
ชุดนี้คือ **ระบบปุ้มปุ้ย v9.3.5 ฉบับเต็ม (source + corpus + tests + INVARIANTS + baseline)** สำหรับป้อนเข้า **Claude Code** เพื่อ harden ต่อ.
...
3. Claude Code จะทำตามขั้น: ติดตั้ง dep (มีเน็ต) → ด่าน sanity (ต้องได้ golden `814c7cf0`) → harden 3 รอบ → ผ่าน gate ครบ → re-vendor → `make_release` → ส่งระบบสมบูรณ์.
...
ทุกอย่างที่จำเป็นต่อการ reproduce golden `814c7cf0` (โค้ด + `corpus/` 146 ไฟล์ + `baseline.json` + `constraints.txt`) อยู่ครบในชุดนี้.

...
_INT_FIELDS: dict[str, tuple[int, int]] = {
    ...
    # Graded trades of experience before the analyst re-tunes itself (>=1).
    "adapt_after_trades": (1, 100_000),
}
...
def order_over_hard_cap(notional_thb: object) -> str | None:
    ...
    # Reject non-finite notionals (NaN / ±Infinity). Decimal("nan") parses
    # cleanly but every comparison against it raises InvalidOperation, and
    # Decimal("-Infinity") would slip UNDER the cap — both are the wrong failure
    # mode for a money-spending guard, so they are refused outright.
    if not value.is_finite():
        return f"non-finite order notional {notional_thb!r} — rejected"
...
class RiskSettings:
    ...
    # Graded trades of experience before the analyst re-tunes its own selectivity.
    ...
    def as_str_dict(self) -> dict[str, str]:
        return {
            ...
            "adapt_after_trades": str(self.adapt_after_trades),
        }
...
def validate_risk_settings(
    ...
    return (
        RiskSettings(
            ...
            adapt_after_trades=int_out["adapt_after_trades"],
        ),
        [],
    )
